...
// app/unit-tests/functions/hmrcItsaCrystallisationObligationsGet.test.js
...
import { ingestHandler as hmrcItsaCrystallisationObligationsGetHandler } from "@app/functions/hmrc/hmrcItsaCrystallisationObligationsGet.js";
...
describe("hmrcItsaCrystallisationObligationsGet ingestHandler", () => {
  beforeEach(() => {
  ...
  test("returns 400 for an invalid status value", async () => {
    const event = buildHmrcEvent({
      queryStringParameters: { nino: VALID_NINO, status: "not-a-status" },
      headers: { authorization: "Bearer test-token" },
    });
    const response = await hmrcItsaCrystallisationObligationsGetHandler(event);
    expect(response.statusCode).toBe(400);
  ...
  test("returns 200 with obligations on success, without imposing a fixed tax year", async () => {
    const obligations = {
      obligations: [
        {
  ...
  test("calls HMRC with the v3.0 Accept header", async () => {
    ...
    await hmrcItsaCrystallisationObligationsGetHandler(event);
  ...
  test("calls the correct HMRC endpoint path with the NINO", async () => {
    ...
    expect(calledUrl).toContain(`/obligations/details/${VALID_NINO}/crystallisation`);
  });

  test("forwards taxYear and status filters to HMRC", async () => {
    mockHmrcSuccess(mockFetch, { obligations: [] });

    const event = buildHmrcEvent({
      queryStringParameters: { nino: VALID_NINO, taxYear: "2023-24", status: "open" },
      headers: { authorization: "Bearer test-token" },
    });
    await hmrcItsaCrystallisationObligationsGetHandler(event);

    const calledUrl = mockFetch.mock.calls[0][0];
    expect(calledUrl).toContain("taxYear=2023-24");
    expect(calledUrl).toContain("status=open");
  });

  test("publishes the itsa-crystallisation-obligations-queried event with the hashed sub, never the raw sub", async () => {
    mockHmrcSuccess(mockFetch, { obligations: [] });
    ...
    const queriedCalls = mockEventBridgeSend.mock.calls.filter((call) => {
      ...
      return detail.event === "itsa-crystallisation-obligations-queried";
    });
  ...
  test("returns 403 JSON when the authenticated user holds no bundle entitled to ITSA obligations", async () => {
    ...
    event.requestContext.http.path = "/api/v1/hmrc/itsa/obligations/crystallisation";
    const response = await hmrcItsaCrystallisationObligationsGetHandler(event);
...
import { workerHandler as hmrcItsaCrystallisationObligationsGetWorker } from "@app/functions/hmrc/hmrcItsaCrystallisationObligationsGet.js";

describe("hmrcItsaCrystallisationObligationsGet worker", () => {
  beforeEach(() => {
    Object.assign(process.env, setupTestEnv());
    vi.clearAllMocks();
  });
  ...
  test("successfully processes SQS message and marks as completed", async () => {
    ...
    await hmrcItsaCrystallisationObligationsGetWorker(event);
