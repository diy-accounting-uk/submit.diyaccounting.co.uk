...
// app/unit-tests/functions/hmrcItsaLossesAndClaimsDelete.activity.test.js
...
function buildDeleteBody(overrides = {}) {
  return {
    nino: VALID_NINO,
...
describe("hmrcItsaLossesAndClaimsDelete token charge", () => {
  beforeAll(async () => {
  ...
  test("checks token availability for the self-employed-year-end-delete activity on the initial request, before HMRC is called", async () => {
    ...
    const response = await hmrcItsaLossesAndClaimsDeleteHandler(buildInitialDeleteEvent());
  ...
  test("an HMRC rejection never charges a token", async () => {
    ...
    await hmrcItsaLossesAndClaimsDeleteHandler(buildInitialDeleteEvent());
