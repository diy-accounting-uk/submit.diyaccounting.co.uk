...
import { ingestHandler as hmrcVatPaymentsGetHandler } from "@app/functions/hmrc/hmrcVatPaymentsGet.js";
...
describe("hmrcVatPaymentsGet ingestHandler", () => {
  beforeEach(() => {
  ...
  test("returns 200 with payments list on success", async () => {
    const payments = {
      payments: [{ amount: 1000.0, received: "2024-05-06" }],
    };
    mockHmrcSuccess(mockFetch, payments);
  ...
  test("publishes the vat-payments-queried event with the hashed sub, never the raw sub", async () => {
    const payments = { payments: [{ amount: 1000.0, received: "2024-05-06" }] };
    mockHmrcSuccess(mockFetch, payments);
  ...
  test("returns 403 JSON when the authenticated user holds no bundle entitled to VAT payments", async () => {
    // QueryCommand (bundle lookup) resolves to no items via the default beforeEach mock,
    // so the user has only the automatic "default" bundle, which vat-payments does not accept.
    const event = buildHmrcEvent({
      queryStringParameters: { vrn: "111222333" },
      headers: { authorization: "Bearer test-token" },
    });
    event.requestContext.http.path = "/api/v1/hmrc/vat/payment";
    const response = await hmrcVatPaymentsGetHandler(event);
  ...
  test("allows the request through once the user holds a bundle entitled to VAT payments", async () => {
    mockSend.mockImplementation(async (cmd) => {
      const lib = await import("@aws-sdk/lib-dynamodb");
      if (cmd instanceof lib.QueryCommand) {
  ...
  test("returns 200 when processing completes synchronously (large x-wait-time-ms)", async () => {
    ...
    const response = await hmrcVatPaymentsGetHandler(event);
    expect(response.statusCode).toBe(200);
    expect(JSON.parse(response.body)).toEqual(payments);
  });
});

import { workerHandler as hmrcVatPaymentsGetWorker } from "@app/functions/hmrc/hmrcVatPaymentsGet.js";

describe("hmrcVatPaymentsGet worker", () => {
  beforeEach(() => {
    Object.assign(process.env, setupTestEnv());
    vi.clearAllMocks();
  });
  ...
  test("successfully processes SQS message and marks as completed", async () => {
    ...
    await hmrcVatPaymentsGetWorker(event);
    ...
    expect(completedCall[0].input.ExpressionAttributeValues[":data"].payments).toEqual(payments);
  });
});
