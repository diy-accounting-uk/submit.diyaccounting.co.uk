...
// app/unit-tests/functions/hmrcItsaUkPropertyAnnualPut.activity.test.js
...
import { ingestHandler as hmrcItsaUkPropertyAnnualPutHandler } from "@app/functions/hmrc/hmrcItsaUkPropertyAnnualPut.js";
...
describe("hmrcItsaUkPropertyAnnualPut token charge", () => {
  beforeAll(async () => {
  ...
  test("checks token availability for the self-employed-year-end activity on the initial request, before HMRC is called", async () => {
    ...
    const response = await hmrcItsaUkPropertyAnnualPutHandler(buildInitialAnnualEvent());
  ...
  test("an HMRC rejection never charges a token", async () => {
    ...
    await hmrcItsaUkPropertyAnnualPutHandler(buildInitialAnnualEvent());
