...
// app/unit-tests/functions/hmrcItsaSelfEmploymentAnnualGet.test.js
...
import { ingestHandler as hmrcItsaSelfEmploymentAnnualGetHandler } from "@app/functions/hmrc/hmrcItsaSelfEmploymentAnnualGet.js";
...
describe("hmrcItsaSelfEmploymentAnnualGet ingestHandler", () => {
  beforeEach(() => {
  ...
  test("returns 400 when taxYear is missing", async () => {
    ...
    const response = await hmrcItsaSelfEmploymentAnnualGetHandler(event);
  ...
  test("calls the correct HMRC endpoint path with nino, businessId and taxYear", async () => {
    ...
    await hmrcItsaSelfEmploymentAnnualGetHandler(event);
  ...
  test("publishes the itsa-annual-submission-queried event with the hashed sub, never the raw sub", async () => {
    mockHmrcSuccess(mockFetch, {});
    ...
    const queriedCalls = mockEventBridgeSend.mock.calls.filter((call) => {
      ...
      return detail.event === "itsa-annual-submission-queried";
    });
  ...
  test("returns 200 when processing completes synchronously (large x-wait-time-ms)", async () => {
    const annualSubmission = { adjustments: { includedNonTaxableProfits: 200 } };
    mockHmrcSuccess(mockFetch, annualSubmission);
...
import { workerHandler as hmrcItsaSelfEmploymentAnnualGetWorker } from "@app/functions/hmrc/hmrcItsaSelfEmploymentAnnualGet.js";

describe("hmrcItsaSelfEmploymentAnnualGet worker", () => {
  beforeEach(() => {
    Object.assign(process.env, setupTestEnv());
    vi.clearAllMocks();
  });
  ...
  test("successfully processes SQS message and marks as completed", async () => {
    ...
    await hmrcItsaSelfEmploymentAnnualGetWorker(event);
    ...
    expect(completedCall[0].input.ExpressionAttributeValues[":data"].annualSubmission).toEqual(annualSubmission);
  });
});
