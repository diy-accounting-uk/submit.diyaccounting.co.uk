...
// app/unit-tests/functions/hmrcItsaObligationsGet.test.js
...
import { ingestHandler as hmrcItsaObligationsGetHandler } from "@app/functions/hmrc/hmrcItsaObligationsGet.js";
...
describe("hmrcItsaObligationsGet ingestHandler", () => {
  beforeEach(() => {
  ...
  test("HEAD request returns 200 OK", async () => {
    ...
    const response = await hmrcItsaObligationsGetHandler(event);
  ...
  test("returns 400 for an invalid typeOfBusiness value", async () => {
    const event = buildHmrcEvent({
      queryStringParameters: { nino: VALID_NINO, typeOfBusiness: "not-a-business-type" },
      headers: { authorization: "Bearer test-token" },
    });
    const response = await hmrcItsaObligationsGetHandler(event);
    expect(response.statusCode).toBe(400);
  ...
  test("returns 400 when businessId is supplied without typeOfBusiness", async () => {
    const event = buildHmrcEvent({
      queryStringParameters: { nino: VALID_NINO, businessId: "XAIS12345678901" },
      headers: { authorization: "Bearer test-token" },
    });
    const response = await hmrcItsaObligationsGetHandler(event);
    expect(response.statusCode).toBe(400);
  ...
  test("returns 200 with obligations on success, with no default date range imposed", async () => {
    const obligations = {
      obligations: [
        {
    ...
    // Never impose a fromDate/toDate default: HMRC obligations are unpredictable and the
    // simulator/tests must not encourage hardcoding a date window.
    const calledUrl = mockFetch.mock.calls[0][0];
    expect(calledUrl).not.toContain("fromDate=");
    expect(calledUrl).not.toContain("toDate=");
  });

  test("calls HMRC with the v3.0 Accept header", async () => {
    mockHmrcSuccess(mockFetch, { obligations: [] });
    ...
    await hmrcItsaObligationsGetHandler(event);

    expect(mockFetch).toHaveBeenCalled();
    const calledInit = mockFetch.mock.calls[0][1];
    expect(calledInit.headers.Accept).toBe("application/vnd.hmrc.3.0+json");
  });
  ...
  test("calls the correct HMRC endpoint path with the NINO", async () => {
    ...
    const calledUrl = mockFetch.mock.calls[0][0];
    expect(calledUrl).toContain(`/obligations/details/${VALID_NINO}/income-and-expenditure`);
  });

  test("forwards typeOfBusiness, businessId and status filters to HMRC", async () => {
    mockHmrcSuccess(mockFetch, { obligations: [] });
  ...
  test("publishes the itsa-obligations-queried event with the hashed sub, never the raw sub", async () => {
    mockHmrcSuccess(mockFetch, { obligations: [] });
    ...
    const queriedCalls = mockEventBridgeSend.mock.calls.filter((call) => {
      ...
      return detail.event === "itsa-obligations-queried";
    });
  ...
  test("returns 400 when HMRC answers NO_OBLIGATIONS_FOUND", async () => {
    mockHmrcError(mockFetch, 404, { code: "NO_OBLIGATIONS_FOUND" });
  ...
  test("returns 403 JSON when the authenticated user holds no bundle entitled to ITSA obligations", async () => {
    ...
    event.requestContext.http.path = "/api/v1/hmrc/itsa/obligations";
    const response = await hmrcItsaObligationsGetHandler(event);
  ...
  test("allows the request through once the user holds a bundle entitled to ITSA obligations", async () => {
    mockSend.mockImplementation(async (cmd) => {
      const lib = await import("@aws-sdk/lib-dynamodb");
      if (cmd instanceof lib.QueryCommand) {
  ...
  test("returns 200 when processing completes synchronously (large x-wait-time-ms)", async () => {
    const obligations = { obligations: [{ typeOfBusiness: "self-employment", businessId: "XAIS12345678901", obligationDetails: [] }] };
    mockHmrcSuccess(mockFetch, obligations);
...
import { workerHandler as hmrcItsaObligationsGetWorker } from "@app/functions/hmrc/hmrcItsaObligationsGet.js";

describe("hmrcItsaObligationsGet worker", () => {
  beforeEach(() => {
    Object.assign(process.env, setupTestEnv());
    vi.clearAllMocks();
  });
  ...
  test("successfully processes SQS message and marks as completed", async () => {
    ...
    await hmrcItsaObligationsGetWorker(event);
