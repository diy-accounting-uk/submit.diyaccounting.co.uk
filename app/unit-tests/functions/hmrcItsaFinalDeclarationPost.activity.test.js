...
// app/unit-tests/functions/hmrcItsaFinalDeclarationPost.activity.test.js
...
describe("hmrcItsaFinalDeclarationPost token cost, receipt and failure reporting", () => {
  beforeAll(async () => {
  ...
  test("charges one token for the self-employed activity on the initial request, before HMRC is called", async () => {
    ...
    const response = await hmrcItsaFinalDeclarationPostHandler(buildInitialDeclarationEvent());
  ...
  test("a successful declaration stores a receipt keyed on the calculationId, carrying the confirmed figure and HMRC's correlation id", async () => {
    ...
    expect(receipts[0].receiptId.endsWith(`-${VALID_CALCULATION_ID}`)).toBe(true);
  ...
  test("a successful declaration emits a success metric alongside the submitted event", async () => {
    mockHmrcSuccess(mockFetch, {});

    await hmrcItsaFinalDeclarationPostHandler(buildInitialDeclarationEvent());

    expect(mockPublishActivityEvent).toHaveBeenCalledWith(
      expect.objectContaining({ event: "itsa-final-declaration-submitted", userSub: "test-sub" }),
    );
  ...
  test("an HMRC rejection emits a failure event and a failure metric", async () => {
    ...
    expect(rejections[0][0]).toMatchObject({ event: "itsa-final-declaration-failed", detail: { hmrcStatus: 400 } });
    expect(metricCalls("ItsaSubmissionFailure")).toHaveLength(1);
