...
// app/unit-tests/migrations/006-drop-sandbox-qualifier.test.js
...
import { up, phase } from "../../../scripts/migrations/006-drop-sandbox-qualifier.js";

describe("006-drop-sandbox-qualifier", () => {
  ...
  test("runs post-deploy", () => {
    expect(phase).toBe("post-deploy");
  });
  ...
  test("real run backfills qualifiers.synthetic and removes qualifiers.sandbox, keyed by hashedSub and bundleId", async () => {
    ...
    expect(firstUpdate.UpdateExpression).toBe(
      "SET qualifiers.synthetic = if_not_exists(qualifiers.synthetic, :v) REMOVE qualifiers.sandbox",
    );
    expect(firstUpdate.ExpressionAttributeValues[":v"]).toBe(true);
    expect(firstUpdate.ConditionExpression).toBe("attribute_exists(qualifiers.sandbox)");
