...
// app/unit-tests/migrations/004-backfill-stripe-test-mode.test.js
...
import { up, phase } from "../../../scripts/migrations/004-backfill-stripe-test-mode.js";

describe("004-backfill-stripe-test-mode", () => {
  ...
  test("dry run counts matches, logs the first ten keys, and writes nothing", async () => {
    ...
    expect(scanInput.FilterExpression).toContain("attribute_exists(stripeSubscriptionId)");
    expect(scanInput.FilterExpression).toContain("attribute_exists(qualifiers.sandbox)");
    expect(scanInput.FilterExpression).toContain("attribute_not_exists(qualifiers.stripeTestMode)");
  });

  test("real run copies qualifiers.sandbox onto qualifiers.stripeTestMode, keyed by hashedSub and bundleId", async () => {
    delete process.env.MIGRATION_DRY_RUN;

    mockDynamoSend.mockResolvedValueOnce({
      Items: [
        { hashedSub: "hash-a", bundleId: "resident-pro", stripeSubscriptionId: "sub_a", qualifiers: { sandbox: true } },
        { hashedSub: "hash-b", bundleId: "resident-pro", stripeSubscriptionId: "sub_b", qualifiers: { sandbox: false } },
      ],
    });
    ...
    expect(firstUpdate.UpdateExpression).toBe("SET qualifiers.stripeTestMode = :v");
  ...
  test("pages through ScanCommand results via LastEvaluatedKey", async () => {
    ...
    mockDynamoSend.mockResolvedValueOnce({
      Items: [{ hashedSub: "hash-a", bundleId: "resident-pro", stripeSubscriptionId: "sub_a", qualifiers: { sandbox: true } }],
      LastEvaluatedKey: { hashedSub: "hash-a", bundleId: "resident-pro" },
    });
    mockDynamoSend.mockResolvedValueOnce({}); // UpdateCommand for hash-a
    mockDynamoSend.mockResolvedValueOnce({
      Items: [{ hashedSub: "hash-b", bundleId: "resident-pro", stripeSubscriptionId: "sub_b", qualifiers: { sandbox: true } }],
    });
    mockDynamoSend.mockResolvedValueOnce({}); // UpdateCommand for hash-b
