...
// app/functions/hmrc/hmrcItsaSelfEmploymentPeriodsGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaSelfEmploymentPeriodsGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/self-employment/periods", ingestHandler);
  app.head("/api/v1/hmrc/itsa/self-employment/periods", async (httpRequest, httpResponse) => {
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_SELF_EMPLOYMENT_PERIODS_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { periodSummaries, hmrcResponse } = await getItsaSelfEmploymentPeriods(
          payload.nino,
          payload.businessId,
          payload.taxYear,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during self-employment period summaries retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function getItsaSelfEmploymentPeriods(
  nino,
  businessId,
  taxYear,
  hmrcAccessToken,
  ...
  const hmrcRequestUrl = `/individuals/business/self-employment/${nino}/${businessId}/period/${taxYear}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  await publishActivityEvent({
    event: "itsa-self-employment-periods-queried",
    summary: "ITSA self-employment period summaries listed",
