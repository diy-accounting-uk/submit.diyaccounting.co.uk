...
// app/functions/hmrc/hmrcItsaObligationsGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaObligationsGet.js" });

const MAX_WAIT_MS = 25000;
const DEFAULT_WAIT_MS = 0;

// Obligations (MTD) v3.0 - the API version this endpoint requires.
const HMRC_API_VERSION = "3.0";

const VALID_TYPES_OF_BUSINESS = ["self-employment", "uk-property", "foreign-property"];
...
export function apiEndpoint(app) {
  app.get("/api/v1/hmrc/itsa/obligations", async (httpRequest, httpResponse) => {
    const lambdaEvent = buildLambdaEventFromHttpRequest(httpRequest);
    const lambdaResult = await ingestHandler(lambdaEvent);
    return buildHttpResponseFromLambdaResult(lambdaResult, httpResponse);
  });
  app.head("/api/v1/hmrc/itsa/obligations", async (httpRequest, httpResponse) => {
...
export function extractAndValidateParameters(event, errorMessages) {
  ...
  const { nino, typeOfBusiness, businessId, fromDate, toDate, status, runFraudPreventionHeaderValidation } = queryParams;
  ...
  if (businessId && !BUSINESS_ID_PATTERN.test(businessId)) {
    errorMessages.push("Invalid businessId format");
  }
  if (businessId && !typeOfBusiness) {
    errorMessages.push("typeOfBusiness must be provided when businessId is supplied");
  }
  ...
  if (fromDate && !toDate) errorMessages.push("Missing toDate parameter - required when fromDate is supplied");
  if (toDate && !fromDate) errorMessages.push("Missing fromDate parameter - required when toDate is supplied");
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_OBLIGATIONS_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  const { nino, typeOfBusiness, businessId, fromDate, toDate, status, testScenario, hmrcAccount, runFraudPreventionHeaderValidation } =
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { obligations, hmrcResponse } = await getItsaObligations(
          payload.nino,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({ message: "Unexpected error during ITSA obligations retrieval", error: error.message, stack: error.stack });
      return http500ServerErrorResponse({
        request,
        headers: { ...responseHeaders },
...
export async function getItsaObligations(
  nino,
  hmrcAccessToken,
  govClientHeaders,
  testScenario,
  ...
  const hmrcRequestUrl = `/obligations/details/${nino}/income-and-expenditure`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  await publishActivityEvent({
    event: "itsa-obligations-queried",
    summary: "ITSA obligations queried",
