...
// app/functions/hmrc/hmrcItsaSelfEmploymentPeriodGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaSelfEmploymentPeriodGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/self-employment/period", ingestHandler);
}
/* v8 ignore stop */

export function extractAndValidateParameters(event, errorMessages) {
  const queryParams = event.queryStringParameters || {};
  const { nino, businessId, taxYear, periodId, runFraudPreventionHeaderValidation } = queryParams;
  ...
  // The dated model retrieves one period summary by its periodId; the cumulative model
  ...
  if (submissionModel === "dated" && !periodId) errorMessages.push("Missing periodId parameter");
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_SELF_EMPLOYMENT_PERIOD_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  const { nino, businessId, taxYear, periodId, testScenario, hmrcAccount, runFraudPreventionHeaderValidation } =
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { periodSummary, hmrcResponse } = await getItsaSelfEmploymentPeriod(
          payload.nino,
          payload.businessId,
          payload.taxYear,
          payload.periodId,
        ...
        return { periodSummary, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during self-employment period retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { periodSummary, hmrcResponse: serializableHmrcResponse };
...
export async function getItsaSelfEmploymentPeriod(
  nino,
  businessId,
  taxYear,
  periodId,
  ...
  if (!hmrcResponse.ok) {
    return { hmrcResponse, periodSummary: null };
  }
  await publishActivityEvent({
    event: "itsa-self-employment-period-queried",
    summary: "ITSA self-employment period summary retrieved",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, periodSummary: hmrcResponse.data, hmrcRequestUrl };
}
