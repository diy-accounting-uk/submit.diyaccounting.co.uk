...
/**
 * Record a failed ITSA quarterly update attempt: one business metric and one activity event.
 *
...
export async function ingestHandler(event) {
  ...
  try {
    ...
    await recordSubmissionFailure({ failure: "access-denied", summary: "ITSA quarterly update blocked: no entitlement" });
  ...
  if (isInitialRequest) {
    ...
    try {
      ...
      await recordSubmissionFailure({
        ...
        summary: "ITSA quarterly update failed while checking the submission allowance",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
  ...
  try {
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA quarterly update failed unexpectedly",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA quarterly update failed in the background worker",
        userSub,
      });
      if (userSub && requestId) {
...
export async function createSelfEmploymentPeriod(
  ...
  if (!hmrcResponse.ok) {
    await recordSubmissionFailure({
      ...
      summary: "ITSA quarterly update rejected by HMRC",
