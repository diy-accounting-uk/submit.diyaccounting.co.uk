...
// app/functions/hmrc/hmrcVatPaymentsGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcVatPaymentsGet.js" });
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_VAT_PAYMENTS_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { payments, hmrcResponse } = await getVatPayments(
          payload.vrn,
        ...
        return { payments, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({ message: "Unexpected error during VAT payments retrieval", error: error.message, stack: error.stack });
      return http500ServerErrorResponse({
        request,
        headers: { ...responseHeaders },
  ...
  return asyncApiServices.respond({
    ...
    requestId,
    responseHeaders,
    data: result ? result.payments : null,
  });
}
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { payments, hmrcResponse: serializableHmrcResponse };
...
export async function getVatPayments(
  ...
  const hmrcRequestUrl = `/organisations/vat/${vrn}/payments`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  await publishActivityEvent({
    event: "vat-payments-queried",
    summary: "VAT payments queried",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, payments: hmrcResponse.data, hmrcRequestUrl };
}
