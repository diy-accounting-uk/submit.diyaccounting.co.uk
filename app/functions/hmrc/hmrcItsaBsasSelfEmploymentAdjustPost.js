...
// app/functions/hmrc/hmrcItsaBsasSelfEmploymentAdjustPost.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaBsasSelfEmploymentAdjustPost.js" });
 ...
 * buildBsasAdjustRequestBody and turned into a 400 by the caller, the same way
 * hmrcItsaSelfEmploymentAnnualPut.js's AnnualSubmissionValidationError works.
 */
export class BsasAdjustValidationError extends Error {
  constructor(code, message) {
    super(message);
    this.name = "BsasAdjustValidationError";
    this.code = code;
  }
}
...
/**
 * Build a money-valued section (income, expenses or additions) for the HMRC request body.
 * HMRC's spec makes each of these objects optional, but rejects an empty object at its path -
 * so a section with no entered values must be left out of the body entirely, never sent as {}.
...
/**
 * Build the Business Source Adjustable Summary v7.0 "Submit Self-Employment Accounting
 * Adjustments" request body: either income/expenses/additions, or zeroAdjustments on its own.
 * HMRC rejects a body carrying both forms with RULE_BOTH_ADJUSTMENTS_SUPPLIED and a body
 * carrying neither with RULE_INCORRECT_OR_EMPTY_BODY_SUBMITTED, so both are rejected here
 * rather than sent, the same way buildAnnualSubmissionRequestBody rejects both allowance forms
 * at once.
 * @param {Object} adjustDetails - income, expenses, additions, zeroAdjustments
 * @returns {Object} the HMRC request body
 */
export function buildBsasAdjustRequestBody(adjustDetails) {
  const income = buildMoneySection(adjustDetails.income);
  const expenses = buildMoneySection(adjustDetails.expenses);
  const additions = buildMoneySection(adjustDetails.additions);
  const hasAdjustments = Boolean(income) || Boolean(expenses) || Boolean(additions);
  ...
  if (hasZeroAdjustments) {
    return { zeroAdjustments: true };
  }

  const hmrcRequestBody = {};
  if (income) hmrcRequestBody.income = income;
  if (expenses) hmrcRequestBody.expenses = expenses;
  if (additions) hmrcRequestBody.additions = additions;
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "post", "/api/v1/hmrc/itsa/bsas/self-employment/adjust", ingestHandler);
}
/* v8 ignore stop */

export function extractAndValidateParameters(event, errorMessages) {
  const parsedBody = parseRequestBody(event);
  const { nino, calculationId, taxYear, income, expenses, additions, zeroAdjustments, runFraudPreventionHeaderValidation } =
    parsedBody || {};
  ...
  try {
    buildBsasAdjustRequestBody({ income, expenses, additions, zeroAdjustments });
  } catch (error) {
    if (error instanceof BsasAdjustValidationError) {
  ...
  return {
    ...
    calculationId,
    taxYear,
    // Pass through whatever the caller sent and let buildBsasAdjustRequestBody drop any of
    // income/expenses/additions that end up empty - HMRC rejects an empty object at any of
    // their paths.
    income: income || {},
    expenses: expenses || {},
    additions: additions || {},
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_BSAS_SELF_EMPLOYMENT_ADJUST_POST_ASYNC_REQUESTS_TABLE_NAME;
  ...
  const { nino, calculationId, taxYear, income, expenses, additions, zeroAdjustments, hmrcAccount, runFraudPreventionHeaderValidation } =
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { adjustResult, hmrcResponse, hmrcResponseBody } = await adjustBsasSelfEmployment(
          payload.nino,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({ message: "Unexpected error during BSAS self-employment adjustment", error: error.message, stack: error.stack });
      return http500ServerErrorResponse({
        request,
        headers: { ...responseHeaders },
  ...
  // way hmrcItsaSelfEmploymentAnnualPut.js's putItsaSelfEmploymentAnnual does.
...
export async function adjustBsasSelfEmployment(
  ...
  const hmrcRequestBody = buildBsasAdjustRequestBody(adjustDetails);
  ...
  const hmrcRequestUrl = `${hmrcBase}/individuals/self-assessment/adjustable-summary/${nino}/self-employment/${calculationId}/adjust/${taxYear}`;
  let hmrcResponse = {};
  ...
  await publishActivityEvent({
    event: "itsa-bsas-adjusted",
    summary: "ITSA business source adjustable summary adjusted",
