...
// app/functions/hmrc/hmrcItsaUkPropertyPeriodPut.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaUkPropertyPeriodPut.js" });
...
/**
 * Record a failed ITSA UK property quarterly update amendment: one business metric and one
 * activity event.
 *
...
/**
 * Build the Property Business v6.0 "Amend a UK Property Period Summary" request body. Unlike
 * the create-period-summary body, there is no fromDate/toDate - the period is already
 * identified by the taxYear and submissionId in the request path.
 * @param {Object} periodDetails - ukFhlProperty, ukNonFhlProperty
 * @returns {Object} the HMRC request body
 */
export function buildAmendUkPropertyPeriodRequestBody(periodDetails) {
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "put", "/api/v1/hmrc/itsa/uk-property/period", ingestHandler);
}
/* v8 ignore stop */
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_UK_PROPERTY_PERIOD_PUT_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    ...
    await recordSubmissionFailure({
      ...
      summary: "ITSA UK property quarterly update amendment blocked: no entitlement",
    });
    ...
    try {
      ...
      if (!tokenResult.consumed) {
        ...
        await recordSubmissionFailure({
          ...
          summary: "ITSA UK property quarterly update amendment blocked: submission allowance used up",
          userSub,
        });
      ...
      await recordSubmissionFailure({
        ...
        summary: "ITSA UK property quarterly update amendment failed while checking the submission allowance",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { periodSummary, hmrcResponse, hmrcResponseBody } = await amendUkPropertyPeriod(
          payload.nino,
          payload.businessId,
          payload.taxYear,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({ message: "Unexpected error during UK property period amendment", error: error.message, stack: error.stack });
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA UK property quarterly update amendment failed unexpectedly",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
  ...
  // message, the way hmrcItsaUkPropertyPeriodPost.js's createUkPropertyPeriod does.
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA UK property quarterly update amendment failed in the background worker",
        userSub,
      });
      if (userSub && requestId) {
...
export async function amendUkPropertyPeriod(
  nino,
  businessId,
  taxYear,
  submissionId,
  ...
  const hmrcRequestBody =
    submissionModel === "cumulative"
      ...
      : buildAmendUkPropertyPeriodRequestBody(periodDetails);
  ...
  if (!hmrcResponse.ok) {
    await recordSubmissionFailure({
      ...
      summary: "ITSA UK property quarterly update amendment rejected by HMRC",
  ...
  await publishActivityEvent({
    event: "itsa-uk-property-period-amended",
