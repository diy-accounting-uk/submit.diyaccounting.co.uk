...
// app/functions/hmrc/hmrcItsaStatusGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaStatusGet.js" });

const MAX_WAIT_MS = 25000;
const DEFAULT_WAIT_MS = 0;

// Self Assessment Individual Details v2.0 - the API version this endpoint requires, a
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/status", ingestHandler);
  app.head("/api/v1/hmrc/itsa/status", async (httpRequest, httpResponse) => {
...
export function extractAndValidateParameters(event, errorMessages) {
  ...
  const { nino, taxYear, futureYears, runFraudPreventionHeaderValidation } = queryParams;
  ...
  const futureYearsBool = futureYears === true || futureYears === "true";
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_STATUS_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  const { nino, taxYear, futureYears, testScenario, hmrcAccount, runFraudPreventionHeaderValidation } = extractAndValidateParameters(
    event,
    errorMessages,
  );
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { itsaStatus, hmrcResponse } = await getItsaStatus(
          payload.nino,
          payload.taxYear,
          payload.futureYears,
        ...
        return { itsaStatus, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({ message: "Unexpected error during ITSA status retrieval", error: error.message, stack: error.stack });
      return http500ServerErrorResponse({
        request,
        headers: { ...responseHeaders },
  ...
  return asyncApiServices.respond({
    ...
    requestId,
    responseHeaders,
    data: result ? result.itsaStatus : null,
  });
}
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { itsaStatus, hmrcResponse: serializableHmrcResponse };
...
export async function getItsaStatus(
  nino,
  taxYear,
  futureYears,
  hmrcAccessToken,
  ...
  const hmrcRequestUrl = `/individuals/person/itsa-status/${nino}/${taxYear}`;
  const hmrcQueryParams = futureYears ? { futureYears: "true" } : {};
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  if (!hmrcResponse.ok) {
    return { hmrcResponse, itsaStatus: null };
  }
  await publishActivityEvent({
    event: "itsa-status-queried",
    summary: "ITSA status queried",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, itsaStatus: hmrcResponse.data, hmrcRequestUrl };
}
