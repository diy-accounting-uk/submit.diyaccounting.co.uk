...
// app/functions/hmrc/hmrcItsaUkPropertyPeriodGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaUkPropertyPeriodGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/uk-property/period", ingestHandler);
}
/* v8 ignore stop */

export function extractAndValidateParameters(event, errorMessages) {
  const queryParams = event.queryStringParameters || {};
  const { nino, businessId, taxYear, submissionId, runFraudPreventionHeaderValidation } = queryParams;
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_UK_PROPERTY_PERIOD_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  const { nino, businessId, taxYear, submissionId, testScenario, hmrcAccount, runFraudPreventionHeaderValidation } =
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { periodSummary, hmrcResponse } = await getItsaUkPropertyPeriod(
          payload.nino,
          payload.businessId,
          payload.taxYear,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during UK property period retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function getItsaUkPropertyPeriod(
  nino,
  businessId,
  taxYear,
  submissionId,
  ...
  await publishActivityEvent({
    event: "itsa-uk-property-period-queried",
    summary: "ITSA UK property period summary retrieved",
