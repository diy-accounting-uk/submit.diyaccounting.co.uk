...
// app/functions/hmrc/hmrcItsaTaxLiabilityAdjustmentsDelete.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaTaxLiabilityAdjustmentsDelete.js" });
...
async function recordSubmissionFailure({ failure, summary, userSub, detail = {} }) {
  ...
  await publishActivityFailureEvent({
    event: "itsa-tax-liability-adjustments-deletion-failed",
    summary,
    failure,
    userSub,
    actor,
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "delete", "/api/v1/hmrc/itsa/tax-liability-adjustments", ingestHandler);
}
/* v8 ignore stop */

export function extractAndValidateParameters(event, errorMessages) {
  const parsedBody = parseRequestBody(event);
  const { nino, taxYear, suspendTemporalValidations, runFraudPreventionHeaderValidation } = parsedBody || {};
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_TAX_LIABILITY_ADJUSTMENTS_DELETE_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    ...
    await recordSubmissionFailure({
      ...
      summary: "ITSA tax liability adjustments deletion blocked: no entitlement",
    });
  ...
  const { nino, taxYear, suspendTemporalValidations, hmrcAccount, runFraudPreventionHeaderValidation } = extractAndValidateParameters(
    event,
    errorMessages,
  );
  ...
  if (isInitialRequest) {
    ...
    try {
      ...
      await recordSubmissionFailure({
        ...
        summary: "ITSA tax liability adjustments deletion failed while checking the submission allowance",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { deleted, hmrcResponse, hmrcResponseBody } = await deleteItsaTaxLiabilityAdjustments(
          payload.nino,
          payload.taxYear,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during tax liability adjustments deletion",
        error: error.message,
        stack: error.stack,
      });
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA tax liability adjustments deletion failed unexpectedly",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA tax liability adjustments deletion failed in the background worker",
        userSub,
      });
      if (userSub && requestId) {
...
export async function deleteItsaTaxLiabilityAdjustments(
  nino,
  taxYear,
  suspendTemporalValidations,
  hmrcAccessToken,
  ...
  if (!hmrcResponse.ok) {
    await recordSubmissionFailure({
      ...
      summary: "ITSA tax liability adjustments deletion rejected by HMRC",
  ...
  await publishActivityEvent({
    event: "itsa-tax-liability-adjustments-deleted",
