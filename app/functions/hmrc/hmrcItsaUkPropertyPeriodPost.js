...
// app/functions/hmrc/hmrcItsaUkPropertyPeriodPost.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaUkPropertyPeriodPost.js" });
...
/**
 * Record a failed ITSA UK property quarterly update attempt: one business metric and one
 * activity event.
 *
...
async function recordSubmissionFailure({ failure, summary, userSub, detail = {} }) {
  ...
  await publishActivityFailureEvent({
    event: "itsa-uk-property-period-failed",
    summary,
    failure,
    userSub,
    actor,
 ...
 * empty object at a section's path with "An empty or non-matching body was submitted" - so a
 * section with no entered values must be left out of the body entirely, never sent as {}.
...
/**
 * Build the "income" section of a UK property income type: the flat money fields plus a nested
 * rentARoom.rentsReceived, included only when the caller entered it.
 * @param {Object|undefined} income
 * @returns {Object|undefined}
 */
function buildPropertyIncomeSection(income) {
  if (!income || typeof income !== "object") return undefined;
  const { rentARoom, ...flatIncome } = income;
  const flatMoney = buildMoneySection(flatIncome);
  const rentARoomMoney = buildMoneySection(rentARoom);
  const hmrcIncome = {};
  if (flatMoney) Object.assign(hmrcIncome, flatMoney);
  if (rentARoomMoney) hmrcIncome.rentARoom = rentARoomMoney;
  return Object.keys(hmrcIncome).length > 0 ? hmrcIncome : undefined;
}

/**
 * Build the "expenses" section of a UK property income type: the flat money fields (which
 * includes consolidatedExpenses, mutually exclusive with the itemised fields - HMRC validates
 * that, the way it validates every other amount, so this passes the caller's fields through
 * unchanged) plus a nested rentARoom.amountClaimed, included only when the caller entered it.
 * @param {Object|undefined} expenses
 * @returns {Object|undefined}
 */
function buildPropertyExpensesSection(expenses) {
  if (!expenses || typeof expenses !== "object") return undefined;
  const { rentARoom, ...flatExpenses } = expenses;
  const flatMoney = buildMoneySection(flatExpenses);
  const rentARoomMoney = buildMoneySection(rentARoom);
  const hmrcExpenses = {};
  if (flatMoney) Object.assign(hmrcExpenses, flatMoney);
  if (rentARoomMoney) hmrcExpenses.rentARoom = rentARoomMoney;
  return Object.keys(hmrcExpenses).length > 0 ? hmrcExpenses : undefined;
}

/**
 * Build one property income type's object (ukFhlProperty or ukNonFhlProperty): its income and
 * expenses sections, included only when the caller entered something in either.
 * @param {Object|undefined} propertyType
 * @returns {Object|undefined}
 */
function buildPropertyTypeSection(propertyType) {
  if (!propertyType || typeof propertyType !== "object") return undefined;
  const income = buildPropertyIncomeSection(propertyType.income);
  const expenses = buildPropertyExpensesSection(propertyType.expenses);
  const hmrcPropertyType = {};
  if (income) hmrcPropertyType.income = income;
  if (expenses) hmrcPropertyType.expenses = expenses;
  return Object.keys(hmrcPropertyType).length > 0 ? hmrcPropertyType : undefined;
}

/**
 * Build the Property Business v6.0 "Create a UK Property Period Summary" request body, for tax
 * years 2024-25 and earlier. fromDate and toDate are always required; ukFhlProperty and
 * ukNonFhlProperty are each included only when the caller entered a value for at least one of
 * their income or expenses fields.
 * @param {Object} periodDetails - fromDate, toDate, ukFhlProperty, ukNonFhlProperty
 * @returns {Object} the HMRC request body
 */
export function buildUkPropertyPeriodRequestBody(periodDetails) {
  const hmrcRequestBody = {
    fromDate: periodDetails.fromDate,
    toDate: periodDetails.toDate,
  };

  const ukFhlProperty = buildPropertyTypeSection(periodDetails.ukFhlProperty);
  if (ukFhlProperty) hmrcRequestBody.ukFhlProperty = ukFhlProperty;

  const ukNonFhlProperty = buildPropertyTypeSection(periodDetails.ukNonFhlProperty);
  if (ukNonFhlProperty) hmrcRequestBody.ukNonFhlProperty = ukNonFhlProperty;
...
export function apiEndpoint(app) {
  app.post("/api/v1/hmrc/itsa/uk-property/period", async (httpRequest, httpResponse) => {
    const lambdaEvent = buildLambdaEventFromHttpRequest(httpRequest);
    const lambdaResult = await ingestHandler(lambdaEvent);
    return buildHttpResponseFromLambdaResult(lambdaResult, httpResponse);
  });
  app.head("/api/v1/hmrc/itsa/uk-property/period", async (httpRequest, httpResponse) => {
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_UK_PROPERTY_PERIOD_POST_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    ...
    await recordSubmissionFailure({ failure: "access-denied", summary: "ITSA UK property quarterly update blocked: no entitlement" });
  ...
  // Token enforcement: consume 1 token for the quarterly update (the "value action") - initial request only
  if (isInitialRequest) {
    ...
    try {
      ...
      if (!tokenResult.consumed) {
        ...
        await recordSubmissionFailure({
          ...
          summary: "ITSA UK property quarterly update blocked: submission allowance used up",
          userSub,
        });
      ...
      await recordSubmissionFailure({
        ...
        summary: "ITSA UK property quarterly update failed while checking the submission allowance",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { periodSummary, hmrcResponse, hmrcResponseBody } = await createUkPropertyPeriod(
          payload.nino,
          payload.businessId,
          payload.taxYear,
          {
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({ message: "Unexpected error during UK property period creation", error: error.message, stack: error.stack });
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA UK property quarterly update failed unexpectedly",
        userSub,
      });
      return http500ServerErrorResponse({
        request,
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      await recordSubmissionFailure({
        failure: "internal-error",
        summary: "ITSA UK property quarterly update failed in the background worker",
        userSub,
      });
      if (userSub && requestId) {
...
export async function createUkPropertyPeriod(
  nino,
  businessId,
  taxYear,
  periodDetails,
  ...
  if (!hmrcResponse.ok) {
    await recordSubmissionFailure({
      ...
      summary: "ITSA UK property quarterly update rejected by HMRC",
  ...
  await publishActivityEvent({
    event: "itsa-uk-property-period-created",
