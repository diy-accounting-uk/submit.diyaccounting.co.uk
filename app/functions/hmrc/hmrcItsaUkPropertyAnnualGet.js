...
// app/functions/hmrc/hmrcItsaUkPropertyAnnualGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaUkPropertyAnnualGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/uk-property/annual", ingestHandler);
}
/* v8 ignore stop */
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_UK_PROPERTY_ANNUAL_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { annualSubmission, hmrcResponse } = await getItsaUkPropertyAnnual(
          payload.nino,
          payload.businessId,
          payload.taxYear,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during UK property annual submission retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function getItsaUkPropertyAnnual(
  nino,
  businessId,
  taxYear,
  hmrcAccessToken,
  ...
  const hmrcRequestUrl = `/individuals/business/property/uk/${nino}/${businessId}/annual/${taxYear}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  await publishActivityEvent({
    event: "itsa-uk-property-annual-submission-queried",
    summary: "ITSA UK property annual submission retrieved",
