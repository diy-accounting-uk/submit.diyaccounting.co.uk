...
// app/functions/hmrc/hmrcItsaCrystallisationObligationsGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaCrystallisationObligationsGet.js" });
...
export function apiEndpoint(app) {
  app.get("/api/v1/hmrc/itsa/obligations/crystallisation", async (httpRequest, httpResponse) => {
    const lambdaEvent = buildLambdaEventFromHttpRequest(httpRequest);
    const lambdaResult = await ingestHandler(lambdaEvent);
    return buildHttpResponseFromLambdaResult(lambdaResult, httpResponse);
  });
  app.head("/api/v1/hmrc/itsa/obligations/crystallisation", async (httpRequest, httpResponse) => {
...
export function extractAndValidateParameters(event, errorMessages) {
  ...
  const { nino, taxYear, status, runFraudPreventionHeaderValidation } = queryParams;
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_CRYSTALLISATION_OBLIGATIONS_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  const { nino, taxYear, status, testScenario, hmrcAccount, runFraudPreventionHeaderValidation } = extractAndValidateParameters(
    event,
    errorMessages,
  );
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { obligations, hmrcResponse } = await getItsaCrystallisationObligations(
          payload.nino,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during ITSA crystallisation obligations retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function getItsaCrystallisationObligations(
  nino,
  hmrcAccessToken,
  govClientHeaders,
  testScenario,
  ...
  const hmrcRequestUrl = `/obligations/details/${nino}/crystallisation`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  await publishActivityEvent({
    event: "itsa-crystallisation-obligations-queried",
    summary: "ITSA final declaration obligations queried",
