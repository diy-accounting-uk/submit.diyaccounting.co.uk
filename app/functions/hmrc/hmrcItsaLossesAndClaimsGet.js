...
// app/functions/hmrc/hmrcItsaLossesAndClaimsGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaLossesAndClaimsGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/losses-and-claims", ingestHandler);
}
/* v8 ignore stop */
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_LOSSES_AND_CLAIMS_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { lossesAndClaims, hmrcResponse } = await getItsaLossesAndClaims(
          payload.nino,
          payload.businessId,
          payload.taxYear,
        ...
        return { lossesAndClaims, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during losses and claims retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { lossesAndClaims, hmrcResponse: serializableHmrcResponse };
...
export async function getItsaLossesAndClaims(
  nino,
  businessId,
  taxYear,
  hmrcAccessToken,
  ...
  const hmrcRequestUrl = `/individuals/losses/${nino}/businesses/${businessId}/loss-claims/${taxYear}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  if (!hmrcResponse.ok) {
    return { hmrcResponse, lossesAndClaims: null };
  }
  await publishActivityEvent({
    event: "itsa-losses-and-claims-queried",
    summary: "ITSA losses and claims retrieved",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, lossesAndClaims: hmrcResponse.data, hmrcRequestUrl };
}
