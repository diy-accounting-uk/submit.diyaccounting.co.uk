...
// app/functions/hmrc/hmrcItsaBsasSelfEmploymentGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaBsasSelfEmploymentGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/bsas/self-employment", ingestHandler);
}
/* v8 ignore stop */
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_BSAS_SELF_EMPLOYMENT_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { bsas, hmrcResponse } = await getItsaBsasSelfEmployment(
          payload.nino,
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during BSAS self-employment retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function getItsaBsasSelfEmployment(
  nino,
  calculationId,
  taxYear,
  hmrcAccessToken,
  ...
  const hmrcRequestUrl = `/individuals/self-assessment/adjustable-summary/${nino}/self-employment/${calculationId}/${taxYear}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  await publishActivityEvent({
    event: "itsa-bsas-queried",
    summary: "ITSA business source adjustable summary retrieved",
