...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaTaxLiabilityAdjustmentsGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/tax-liability-adjustments", ingestHandler);
}
/* v8 ignore stop */

export function extractAndValidateParameters(event, errorMessages) {
  const queryParams = event.queryStringParameters || {};
  const { nino, taxYear, runFraudPreventionHeaderValidation } = queryParams;
...
export async function ingestHandler(event) {
  ...
  const { nino, taxYear, testScenario, hmrcAccount, runFraudPreventionHeaderValidation } = extractAndValidateParameters(
    event,
    errorMessages,
  );
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        ...
        return { taxLiabilityAdjustments, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during tax liability adjustments retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { taxLiabilityAdjustments, hmrcResponse: serializableHmrcResponse };
...
export async function getItsaTaxLiabilityAdjustments(
  nino,
  taxYear,
  hmrcAccessToken,
  govClientHeaders,
  ...
  const hmrcRequestUrl = `/individuals/tax-liability/adjustments/${nino}/${taxYear}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  if (!hmrcResponse.ok) {
    return { hmrcResponse, taxLiabilityAdjustments: null };
  }
  await publishActivityEvent({
    event: "itsa-tax-liability-adjustments-queried",
    summary: "ITSA tax liability adjustments retrieved",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, taxLiabilityAdjustments: hmrcResponse.data, hmrcRequestUrl };
}
