...
// app/functions/hmrc/hmrcItsaCalculationGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaCalculationGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/calculation", ingestHandler);
}
/* v8 ignore stop */
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_CALCULATION_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  const { nino, calculationId, taxYear, testScenario, hmrcAccount, runFraudPreventionHeaderValidation } = extractAndValidateParameters(
    event,
    errorMessages,
  );
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { calculation, hmrcResponse } = await getItsaCalculation(
          payload.nino,
          payload.taxYear,
        ...
        return { calculation, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during ITSA calculation retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { calculation, hmrcResponse: serializableHmrcResponse };
...
export async function getItsaCalculation(
  nino,
  taxYear,
  calculationId,
  hmrcAccessToken,
  ...
  const hmrcRequestUrl = `/individuals/calculations/${nino}/self-assessment/${taxYear}/${calculationId}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  if (!hmrcResponse.ok) {
    return { hmrcResponse, calculation: null };
  }
  await publishActivityEvent({
    event: "itsa-calculation-queried",
    summary: "ITSA tax calculation retrieved",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, calculation: hmrcResponse.data, hmrcRequestUrl };
}
