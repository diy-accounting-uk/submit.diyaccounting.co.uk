...
// app/functions/hmrc/hmrcItsaSelfEmploymentAnnualGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaSelfEmploymentAnnualGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/self-employment/annual", ingestHandler);
}
/* v8 ignore stop */
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_SELF_EMPLOYMENT_ANNUAL_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { annualSubmission, hmrcResponse } = await getItsaSelfEmploymentAnnual(
          payload.nino,
          payload.businessId,
          payload.taxYear,
        ...
        return { annualSubmission, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during self-employment annual submission retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { annualSubmission, hmrcResponse: serializableHmrcResponse };
...
export async function getItsaSelfEmploymentAnnual(
  nino,
  businessId,
  taxYear,
  hmrcAccessToken,
  ...
  const hmrcRequestUrl = `/individuals/business/self-employment/${nino}/${businessId}/annual/${taxYear}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  if (!hmrcResponse.ok) {
    return { hmrcResponse, annualSubmission: null };
  }
  await publishActivityEvent({
    event: "itsa-annual-submission-queried",
    summary: "ITSA self-employment annual submission retrieved",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, annualSubmission: hmrcResponse.data, hmrcRequestUrl };
}
