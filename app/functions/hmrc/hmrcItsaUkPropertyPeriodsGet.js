...
// app/functions/hmrc/hmrcItsaUkPropertyPeriodsGet.js
...
const logger = createLogger({ source: "app/functions/hmrc/hmrcItsaUkPropertyPeriodsGet.js" });
...
export function apiEndpoint(app) {
  registerLambdaRoute(app, "get", "/api/v1/hmrc/itsa/uk-property/periods", ingestHandler);
  app.head("/api/v1/hmrc/itsa/uk-property/periods", async (httpRequest, httpResponse) => {
...
export function extractAndValidateParameters(event, errorMessages) {
  ...
  const { nino, businessId, taxYear, runFraudPreventionHeaderValidation } = queryParams;
  ...
  if (!businessId) errorMessages.push("Missing businessId parameter");
  ...
  // retrieve endpoint (hmrcItsaUkPropertyPeriodGet.js) instead of calling this one.
...
export async function ingestHandler(event) {
  ...
  const asyncRequestsTableName = process.env.HMRC_ITSA_UK_PROPERTY_PERIODS_GET_ASYNC_REQUESTS_TABLE_NAME;
  ...
  try {
    if (persistedRequest) {
      ...
      const processor = async (payload) => {
        const { periodSummaries, hmrcResponse } = await getItsaUkPropertyPeriods(
          payload.nino,
          payload.businessId,
          payload.taxYear,
        ...
        return { periodSummaries, hmrcResponse: serializableHmrcResponse };
      };
    ...
    if (error instanceof asyncApiServices.RequestFailedError) {
      ...
      logger.error({
        message: "Unexpected error during UK property period summaries retrieval",
        error: error.message,
        stack: error.stack,
      });
      return http500ServerErrorResponse({
  ...
  return asyncApiServices.respond({
    ...
    requestId,
    responseHeaders,
    data: result ? result.periodSummaries : null,
  });
}
...
export async function workerHandler(event) {
  ...
  for (const record of event.Records || []) {
    ...
    try {
      ...
      const result = { periodSummaries, hmrcResponse: serializableHmrcResponse };
...
export async function getItsaUkPropertyPeriods(
  nino,
  businessId,
  taxYear,
  hmrcAccessToken,
  ...
  // Untyped path - HMRC serves the same list-period-summaries endpoint for UK and foreign
  // property alike, unlike the typed create/retrieve/amend paths above.
  const hmrcRequestUrl = `/individuals/business/property/${nino}/${businessId}/period/${taxYear}`;
  let hmrcResponse = {};
  /* v8 ignore start */
  ...
  if (!hmrcResponse.ok) {
    return { hmrcResponse, periodSummaries: null };
  }
  await publishActivityEvent({
    event: "itsa-uk-property-periods-queried",
    summary: "ITSA UK property period summaries listed",
    userSub: auditForUserSub,
  });
  return { hmrcResponse, periodSummaries: hmrcResponse.data, hmrcRequestUrl };
}
