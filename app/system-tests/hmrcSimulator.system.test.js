...
describe("HTTP Simulator", () => {
  ...
  describe("ITSA UK Property Cumulative", () => {
    const validBody = () => ({
      fromDate: "2025-04-06",
      toDate: "2025-07-05",
      ukProperty: {
    ...
    it("should create or amend the year's running total with a 204 and no body", async () => {
      const response = await fetch(`${baseUrl}/individuals/business/property/uk/AB123456C/XAIS12345678910/cumulative/2025-26`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "Accept": "application/vnd.hmrc.6.0+json",
    ...
    it("should return 400 for an invalid NINO", async () => {
      const response = await fetch(`${baseUrl}/individuals/business/property/uk/invalid-nino/XAIS12345678910/cumulative/2025-26`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(validBody()),
      });
    ...
    it("should return 400 when ukProperty carries neither income nor expenses", async () => {
      const response = await fetch(`${baseUrl}/individuals/business/property/uk/AB123456C/XAIS12345678910/cumulative/2025-26`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fromDate: "2025-04-06", toDate: "2025-07-05" }),
      });
      expect(response.status).toBe(400);
      const data = await response.json();
      expect(data.code).toBe("RULE_INCORRECT_OR_EMPTY_BODY_SUBMITTED");
    ...
    it("should retrieve a period summary when a scenario is given", async () => {
      const response = await fetch(`${baseUrl}/individuals/business/property/uk/AB123456C/XAIS12345678910/cumulative/2025-26`, {
        headers: { "Gov-Test-Scenario": "UK_PROPERTY_FULL_EXPENSES" },
      });
      expect(response.status).toBe(200);
      const data = await response.json();
