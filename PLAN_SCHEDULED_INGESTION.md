...
**Status: verified live in ci 2026-09-02.** `ga4-event-export-pull` invoked directly wrote
2786 rows for dt=2026-08-31; `ga4_bq_events` distinct session count (12) matches the GA4
Data API's own session count for the same day exactly.
...
**Status: verified live in ci 2026-09-02.** A manual execution of `ci-env-analytics-nightly`
reached `SUCCEEDED` with all five tasks green (execution
`manual-verify-b14-1788310957`). The next Monday scheduled run is still unobserved.
...
**Status: verified live in ci 2026-09-02**, as the "Stripe reconciliation" task inside the
`ci-env-analytics-nightly` execution that also verified phase 3. No code change needed.
`stripeReconcile.js` already
