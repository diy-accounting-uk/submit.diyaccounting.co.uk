...
Status: written 2026-09-09 by the spreadsheets session from `../spreadsheets.diyaccounting.co.uk/PLAN_LICENSING_UPLIFT.md`, which is the plan of record for the whole uplift and holds the decisions, the model and the cross-repository audit. This file carries only what lands in this repository. Blocked on busy: every row here waits for the operator's word, as the operator paused work in this repository on 2026-09-09.
...
| LU-15 (Submit's share) | Urgency 3, after the brand package exists (`PLAN_LICENSING_UPLIFT.md` LU-14): pin `@diy-accounting-uk/brand`, copy assets and tokens at build, import the tokens, delete the local logo, favicon and token copies; the footer, favicon and title conventions read from the words file | the plan of record's LU-14 and H-LU-7 | Sonnet | `package.json`, build scripts, stylesheets, `web/public/` |
