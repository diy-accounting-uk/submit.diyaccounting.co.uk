...
DIY Accounting Limited 06846849: last CS01 25 October 2025, next made up to 21 September 2026, due 5 October 2026 (`NEXT.md` OCS). Two directors, both PSCs.
...
Companies House debits the credit account behind the presenter when the first statement of a payment period is accepted; nothing in the envelope carries payment. So the fee path is: (a) the credit account exists and is linked to E0000052288 (CS-H1); (b) `PaymentPeriodsRequest` tells the page whether this submission will be charged; (c) Submit recovers the £50 from the customer. How (c) works is the operator's decision (CS-H5). The alternatives:

| Option | Shape | Consequence |
|---|---|---|
| A. Pass-through charge | A one-off Stripe Checkout price (`confirmation-statement-fee`, £50) that the submit route requires paid for this company and review date when `PaymentPeriodsRequest` says a fee is due | Exact recovery; one more Stripe product and a paid-fee record; the customer sees Companies House's fee as Companies House's fee |
| B. Inside the subscription | `resident` at £39 a year absorbs the fee | Every filer costs £11 more than they pay; only viable at a higher price point |
| C. Fee plus margin | As A with a higher price | Revenue from the filing itself; the price is a catalogue row |

The first prod filing is the operator's own company, whose fee lands on DIY Accounting's own credit account, so CS-H5 does not gate the proof. Until CS-10 lands, the submit route refuses a filing whose payment period is unpaid unless `COMPANIES_HOUSE_CS_FEE_MODE=operator` is set on the deployment, which is how the prod proof runs.
...
From Cowork's `../REPORT_CH_IDENTITY_VERIFICATION.md` (not yet written), the plan needs:

| Id | Needed from the report | Slots into |
|---|---|---|
| V1 | The exact rule for a director who has not verified by the review date during the transition to 18 November 2026: is the statement rejected, accepted and marked, or accepted with the director omitted? | View 3's blank-row rule; `CS_DIRECTOR_NOT_VERIFIED` |
| V2 | What `NameMismatchReason` is for and when the customer must pick one; whether the register name or the verified name goes in `Person` | View 3's director rows; the builder |
| V3 | Whether a personal code may be supplied to Submit by the company on the director's behalf (the guidance says codes may be shared with an agent who files) and what Submit must say about handling it | The page's hint text; the never-stored rule stays |
| V4 | The PSC path for a director-PSC: the PSC service, VS01 through the gateway, or both; whether the confirmation statement can carry it | CS-13's scope |
| V5 | Jane's code and both directors' dates of birth as the register holds them (runbook task B) | CS-H3 |
| V6 | Whether an ACSP registration is needed for Submit to present codes for customers, now or under the presenter measures from November 2026 | A horizon row in `BACKLOG.md`, no task here |

The verification rows on the page and the `VerificationStatement` block in the builder are complete against the schema now; V1 to V4 change messages and the blank-row rule, not the XML.
...
| CS-10 | Fee collection | Build the option CS-H5 picks: for A or C, a Stripe price row in the catalogue, `stripe-sync.js` run through `/stripe-catalogue-sync`, a paid-fee record keyed by company and review date, the submit route's gate on it | `web/public/submit.catalogue.toml`, `companiesHouseConfirmationStatementPost.js` and its test, `app/functions/billing/*` as the option needs | ~5 | Sonnet | CS-H5, CS-4 | machine-only |
...
| CS-H1 | Credit account | Complete the Companies House credit account application form, send it to `chdfinance@companieshouse.gov.uk`, ask for it to be linked to presenter E0000052288; up to 5 working days. Record the account number in the operator's credentials store | none in the repository | 0 | none | none | human-driven |
| CS-H2 | Email the XML team | On the `xml@companieshouse.gov.uk` thread, after O34d's message: Q1 (endpoint and credentials for the 18 November 2025 schemas; test company data for `CompanyDataRequest`), Q2 (shareholders on a no-change statement), Q4 (authorisation tests and the package reference for `ConfirmationAndVerificationStatement`). Paste the answers into CS-9's row | `../DRAFT_EMAIL_XMLGW_CS01.md` (the draft is Claude Code's, the send is the operator's) | 1 | Haiku for the draft | none | human-driven |
| CS-H3 | Personal codes | Both directors' 11-character codes and their register dates of birth, from runbook task B; held by the operator, typed on the page at filing time, never stored | none | 0 | none | none | human-driven |
...
| CS-H5 | The fee decision | Pick A, B or C in "The fee path"; write the choice into CS-10's row | `NEXT.md` | 1 | none | none | human-driven |
...
Order: CS-1, CS-2, CS-3 in one wave; CS-4 then CS-5, CS-6, CS-8 in one wave; CS-7; CS-9 when CS-H1 and CS-H2 have answered; CS-10 when CS-H5 has; CS-11 after CS-9 and CS-H4; CS-12 when the report lands; CS-13 after CS-9. CS-H1, CS-H2, CS-H3 and CS-H5 can all start now.
