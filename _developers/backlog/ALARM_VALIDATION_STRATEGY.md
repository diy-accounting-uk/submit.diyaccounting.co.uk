...
Every check in a stack feeds that stack's one `{prefix}-{stack}-health` composite alarm, which
...
| Lambda.java | `{prefix}-{stack}-health` (composite, `anyOf` every `check-` alarm in the stack) | n/a | routed via OpsStack's AlarmStateChangeRule |
