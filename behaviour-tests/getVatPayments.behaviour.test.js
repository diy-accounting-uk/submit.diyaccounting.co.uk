...
const screenshotPath = "target/behaviour-test-results/screenshots/vat-payments-behaviour-test";
...
// our own HTTP simulator (app/http-simulator/scenarios/payments.js), not by the real HMRC
...
test.beforeEach(async ({}, testInfo) => {
  testInfo.annotations.push({ type: "test-id", description: "getVatPaymentsBehaviour" });
});
...
async function requestAndVerifyPayments(page, paymentsQuery) {
  await initVatPayments(page, screenshotPath);
  await fillInVatPayments(page, { ...paymentsQuery, runFraudPreventionHeaderValidation }, screenshotPath);
  await submitVatPaymentsForm(page, screenshotPath);
  await verifyVatPaymentsResults(page, paymentsQuery, screenshotPath);
  await goToHomePageUsingMainNav(page, screenshotPath);
}

test("Click through: View VAT payments from HMRC", async ({ page }, testInfo) => {
  ...
  await verifyVatPaymentsResults(page, screenshotPath);
  await goToHomePageUsingMainNav(page, screenshotPath);

  /* ************************************* */
  /*  GET PAYMENTS WITH TEST SCENARIOS  */
  /* ************************************* */
  if (isSyntheticMode()) {
    /**
     ...
     *  - SINGLE_PAYMENT: one payment, dates 2017-01-02..2017-02-02
     *  - MULTIPLE_PAYMENTS: several payments, dates 2017-04-05..2017-12-21
     *  - SINGLE_PAYMENT_2018_19: one payment, dates 2018-01-02..2018-02-02
     *  - MULTIPLE_PAYMENTS_2018_19: several payments, dates 2018-04-05..2018-12-21
     *  - INSOLVENT_TRADER: Client is an insolvent trader
     */
    await requestAndVerifyPayments(page, {
      hmrcVatNumber: testVatNumber,
      hmrcVatPeriodFromDate,
      hmrcVatPeriodToDate,
      /* No test scenario */
    ...
    await requestAndVerifyPayments(page, {
      ...
      hmrcVatPeriodFromDate,
      hmrcVatPeriodToDate,
      testScenario: "SINGLE_PAYMENT",
    });
    await requestAndVerifyPayments(page, {
      hmrcVatNumber: testVatNumber,
      hmrcVatPeriodFromDate,
      hmrcVatPeriodToDate,
      testScenario: "MULTIPLE_PAYMENTS",
    });
    await requestAndVerifyPayments(page, {
      hmrcVatNumber: testVatNumber,
      hmrcVatPeriodFromDate,
      hmrcVatPeriodToDate,
      testScenario: "SINGLE_PAYMENT_2018_19",
    });
    await requestAndVerifyPayments(page, {
      hmrcVatNumber: testVatNumber,
      hmrcVatPeriodFromDate,
      hmrcVatPeriodToDate,
      testScenario: "MULTIPLE_PAYMENTS_2018_19",
    });
    await requestAndVerifyPayments(page, {
      hmrcVatNumber: testVatNumber,
      hmrcVatPeriodFromDate,
  ...
  const keyScreenshotPatterns = [
    "00.*focus.*submitting.*vat.*payments.*form",
    "02.*payments.*submit",
    "04.*payments.*results.*pagedown",
    "00.*focus.*a.*developer.*test.*scenario",
  ];

  const screenshotDescriptions = {
    "00.*focus.*submitting.*vat.*payments.*form": "Filling in VAT payments form",
    "02.*payments.*submit": "Submitting VAT payments form",
    "04.*payments.*results.*pagedown": "Viewing VAT payments results",
    "00.*focus.*a.*developer.*test.*scenario": "Submitting VAT payments form with a test scenario",
  };
  ...
  if (runDynamoDb === "run" || runDynamoDb === "useExisting") {
    ...
    // Assert VAT payments GET request exists and validate key fields
    const paymentsRequests = assertHmrcApiRequestExists(
      hmrcApiRequestsFile,
      "GET",
      `/organisations/vat/${testVatNumber}/payments`,
      "VAT payments retrieval",
    );
    console.log(`[DynamoDB Assertions]: Found ${paymentsRequests.length} VAT payments GET request(s)`);

    expect(paymentsRequests.length).toBeGreaterThan(0);
    let http200OkResults = 0;
    let http400BadRequestResults = 0;
    let http403ForbiddenResults = 0;
    let http404NotFoundResults = 0;
    paymentsRequests.forEach((paymentsRequest, index) => {
      assertEssentialFraudPreventionHeadersPresent(paymentsRequest, `GET payments request ${index + 1}`);
      console.log(`[DynamoDB Assertions]: Validating VAT payments GET request ${index + 1} of ${paymentsRequests.length}`);
      const thisRequestHttp200OkResults = countHmrcApiRequestValues(paymentsRequest, {
        "httpRequest.method": "GET",
        "httpResponse.statusCode": 200,
      });
      if (thisRequestHttp200OkResults === 1) {
        // Check that response body contains payments data
        const responseBody = paymentsRequest.httpResponse.body;
        expect(responseBody).toBeDefined();
        expect(responseBody.payments).toBeDefined();
        console.log("[DynamoDB Assertions]: VAT payments response validated successfully");
      }
      http200OkResults += thisRequestHttp200OkResults;
      http400BadRequestResults += countHmrcApiRequestValues(paymentsRequest, {
        "httpRequest.method": "GET",
        "httpResponse.statusCode": 400,
      });
      http403ForbiddenResults += countHmrcApiRequestValues(paymentsRequest, {
        "httpRequest.method": "GET",
        "httpResponse.statusCode": 403,
      });
      const thisRequestHttp404Results = countHmrcApiRequestValues(paymentsRequest, {
        "httpRequest.method": "GET",
        "httpResponse.statusCode": 404,
      });
      if (thisRequestHttp404Results === 1 && !usingHttpSimulator) {
        // Real HMRC sandbox answers 404 NOT_FOUND when a fresh test user has no payments in
        // the queried range, not when the VRN or endpoint is missing. The handler (see
        // hmrcVatPaymentsGet.js) treats that specific code as an empty result for the
    ...
    // Assert result counts. HMRC's own sandbox behaviour for payment retries is not fully
    ...
    console.log("[DynamoDB Assertions]: VAT Payments GET request results summary:");
    ...
    if (usingHttpSimulator) {
      // The simulator's plain-query scenario returns payments data, so no 404s are expected.
      expect(http404NotFoundResults).toBe(0);
    }
    // In the real sandbox, a fresh test user has no payments in range, so HMRC's 404
    ...
    // Pass userSub to filter to current test user's records (CI DynamoDB contains historical data)
    const hashedSubs = await assertConsistentHashedSub(hmrcApiRequestsFile, "VAT Payments test", { filterByUserSub: userSub });
