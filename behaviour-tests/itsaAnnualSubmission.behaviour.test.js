...
// behaviour-tests/itsaAnnualSubmission.behaviour.test.js
...
import {
  fillInItsaAnnualEdits,
  fillInItsaAnnualLoad,
  fillInItsaBusinessDetails,
  initItsaAnnualSubmission,
  initItsaBusinessDetails,
  submitItsaAnnualLoadForm,
  submitItsaAnnualSaveForm,
  submitItsaBusinessDetailsForm,
  verifyItsaAnnualLoadResults,
  verifyItsaAnnualSaveResults,
...
const screenshotPath = "target/behaviour-test-results/screenshots/itsa-annual-submission-behaviour-test";
...
test.beforeEach(async ({}, testInfo) => {
  testInfo.annotations.push({ type: "test-id", description: "itsaAnnualSubmissionBehaviour" });
});
 ...
 * already displayed on the page. The annual submission endpoint needs this id - it is never
...
test("Click through: Load and save an ITSA Annual Submission with HMRC", async ({ page }, testInfo) => {
  ...
  if (!businessId) {
    throw new Error("Business Details returned no businesses - cannot load an annual submission without a businessId");
  }
  await goToHomePageUsingMainNav(page, screenshotPath);
  ...
  await submitItsaAnnualSaveForm(page, screenshotPath);
  await verifyItsaAnnualSaveResults(page, screenshotPath);
  ...
  const testContext = {
    testId: "itsaAnnualSubmissionBehaviour",
    name: testInfo.title,
    title: "Load and Save an ITSA Annual Submission (HMRC: Self Employment Business GET/PUT)",
    description: "Loads the year's annual submission for a business, enters adjustments and allowances, and saves them to HMRC.",
    hmrcApis: [
      { url: "/api/v1/hmrc/itsa/business/details", method: "GET" },
      { url: "/api/v1/hmrc/itsa/self-employment/annual", method: "GET" },
      { url: "/api/v1/hmrc/itsa/self-employment/annual", method: "PUT" },
  ...
  if (runDynamoDb === "run" || runDynamoDb === "useExisting") {
    ...
    const putRequests = assertHmrcApiRequestExists(
      hmrcApiRequestsFile,
      "PUT",
      `/individuals/business/self-employment/${testNino}/${businessId}/annual/${taxYear}`,
      "ITSA self-employment annual submission",
    );
    console.log(`[DynamoDB Assertions]: Found ${putRequests.length} ITSA self-employment annual PUT request(s)`);

    expect(putRequests.length).toBeGreaterThan(0);
    putRequests.forEach((putRequest, index) => {
      assertEssentialFraudPreventionHeadersPresent(putRequest, `PUT self-employment annual request ${index + 1}`);
    });

    await assertFraudPreventionHeaders(hmrcApiRequestsFile, true, true, false, userSub);

    const hashedSubs = await assertConsistentHashedSub(hmrcApiRequestsFile, "ITSA Annual Submission test", {
