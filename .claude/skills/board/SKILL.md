...
- ci sets self-destruct on their own: `SelfDestructStack` fires on the schedule
  `selfDestructDelayHours` hours after creation (`deploy.yml`'s default is 4; the
...
- prod sets never go on their own: only `destroy-prod.yml` with the deployment name removes
  one. A spare prod set costs $35.28 a month (`../developers/submit/archive/PLAN_COST_OPTIMISATION.md`). `Goes when` for a
