...
- [ ] **CS-10c. The charge in the confirmation statement journey.** On
  ...
  Claude Code. **Model**: Sonnet. **Size**: ~6 files. **In flight**: committed on `claude/arclight-cs10c` (worktree `.claude/worktrees/cs10c`); its simulator behaviour run is in progress.
...
- [ ] **B52n. Activities started and completed on the operator dashboard.** Operator, 2026-09-25: a table on `web/public/operator/dashboard.html` listing every activity available in prod, each with the number started (its button clicked) and completed, under "Last 30 days, Trend, Last 90 days" like the objective blocks. The prod-listed activities are the 16 in `web/public/submit.catalogue.toml` with no `environments` limit or `prod` in it (bundle, submit-vat, vat-obligations, vat-liabilities, vat-payments, vat-penalties, view-vat-return, company-lookup, change-registered-office, change-registered-email, diya-gl-storage, my-receipts, generate-pass-digital, generate-pass-physical, help, operator-dashboard); derive the list from the catalogue at publish time, so a new prod listing joins the table without an edit. Completed: `activity_events` already carries a completion event for most (September customer rows: `vat-return-submitted` 22, `vat-return-failed` 5, `vat-obligations-queried` 83, `vat-return-queried` 9, `vat-penalties-queried` 4, `pass-generated` 47, `companies-house-registered-email-address-filed` 2, `bundle-granted` 31); map each activity to its event in one table in code, and name the activities with none (DIYA-GL saves get `book-saved` from SI-2c). Started: nothing records a button click today (the pages send GA4 `login`, `begin_checkout`, `purchase` and `submit_vat_return` only), so each activity's primary button sends an `activity-started` event with the activity id through a server route on the pattern of `app/functions/account/sessionBeaconPost.js`, so started and completed share the lake, the actor classification and the synthetic exclusion. Then a view with started and completed per activity per day, one observation pair per activity in `app/functions/analytics/operatorSnapshotPublish.js` on the existing windowed-SQL machinery, and the table on the page, escaping everything it writes (the no-unsanitized ratchet is at 64). DIYA-GL's own pages (`../spreadsheets.diyaccounting.co.uk/web/diya-gl.co.uk/public/`) send `activity-started` for their save button in the same change or in SI-2d. Tests: the view and observation unit tests, a browser test for the table, and one per started event. Blocked on SI-2c for the `book-saved` completion; the rest can start. **Source**: operator 2026-09-25; `PLAN_ONE_STOP_DASHBOARD.md`. **Owner**: Claude Code. **Model**: Sonnet. **Size**: ~20 files.
