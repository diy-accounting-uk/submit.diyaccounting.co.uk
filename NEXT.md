...
- [ ] **B30q. Text the operator when prod loses service.** `submit-prod` can send SMS since 2026-09-26: sender ID `DIYACCT` (GB, transactional), the operator's number verified in the End User Messaging sandbox and stored in the SecureString `/submit/prod/operator-sms-number`, and the skill `text-antony-submit-prod` sends one by hand through `pinpoint-sms-voice-v2 send-text-message` (SNS `publish --phone-number` accepted two texts that never arrived; an SNS SMS subscription may fail the same way, so the alarm path needs its delivery proven on a phone, or a small Lambda on the alarm topic that calls `send-text-message`). Remaining: an SMS subscription, reading the number from that parameter, on the SNS topic the prod probe and API-availability alarms publish to (find the topic in `OpsStack.java` and the alarm routing `SubmitApplicationCdkResourceTest` asserts), limited to the alarms that mean customers cannot use the service so the $1-a-month `MonthlySpendLimit` holds; a CDK test for the subscription; proof by one alarm set to ALARM with `aws cloudwatch set-alarm-state` on a ci set. In flight: PR #365 (`claude/impulse-ops`), a prod-only Lambda on OpsStack's `alertTopic` that calls `send-text-message`; its branch deploy run 36212580006 failed in `names` on B30r's held slots and needs a re-run once a slot is free. **Source**: operator, 2026-09-26. **Owner**: Claude Code. **Model**: Sonnet. **Size**: ~3 files.

## Machine-only

- [ ] **B30r. A ci slot stays claimed after last-known-good moves off it.** `deploy.yml` never releases the claim on the set that is last-known-good at the run's end (`release-ci-slot.mjs` header; deploy.yml checks both before running it), and nothing releases it when last-known-good later moves to the other set. On 2026-09-26 the claim on `ci-set2` (run 36205802882, finished 01:10 UTC) survived after `ci-set1` became last-known-good, so with `ci-set1` legitimately held both slots were taken and PR #365's deploy run 36212580006 failed in `names` after its 30-minute claim wait; only `claim-ci-slot.mjs`'s staleness backstop (self-destruct delay plus one hour, 05:42 UTC) would have freed it. Fix in `.github/actions/claim-ci-slot/claim-ci-slot.mjs`'s `isSlotFree` (line ~41): a slot is also free when its claim names a run that is no longer unfinished (reuse `slot-claim-active.mjs`'s run-status read and its `UNFINISHED_RUN_STATUSES`) and the slot is not the environment's last-known-good (`/submit/ci/last-known-good-deployment`). Unit tests beside the existing ones for `isSlotFree`; prove with two branch deploys in a row after merge. **Source**: deploy run 36212580006. **Owner**: Claude Code. **Model**: Sonnet. **Size**: ~2 files.
