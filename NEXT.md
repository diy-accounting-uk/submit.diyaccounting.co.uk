...
- [ ] **B40d.2. Rename the modes to `synthetic`/`live` and give the monitoring vocabulary a
  ...
  **Tracks**: `PLAN_MODE_RENAME.md` is the design; four Sonnet layers, one PR each. Layer 2 (Stripe flag `stripeTestMode` plus migration 004): code complete on `claude/mode-rename-2-stripe-flag` (PR #119); verified after the operator runs migration 004 dry then real on ci and prod. Layer 1 (monitoring vocabulary to `probe`, plus the four alarm docs the cuts left stale): code complete on `claude/mode-rename-1-probe`, PR stacked on the batch branch; verified after its deploy by the `-github-probe-failed` alarms in OK. Layer 3 (HMRC mode value, migrations 005 and 006): running, worktree assigned on dispatch, stacked on layers 1 and 2. Layer 4 (docs and copy) follows, and also recomputes the stale alarm-count arithmetic layer 1 left in `REPORT_ALARM_AUDIT.md` and `PLAN_COST_OPTIMISATION.md`.
