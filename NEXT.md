...
- [ ] **(B43a) Instrument the AWS accounts for cost analysis.** Design is at
  ...
  Open steps: the root deploy creating the cost-reporting stacks is running (dispatched
  by the operator, watched); on green, confirm the CUR export, budgets and anomaly
  monitor exist; operator turns on hourly granularity in Cost Management Preferences
  (console only). Feeds backlog #43's
