...
- [ ] **B30at1. The sweep's claim check, proven.** The sweep reads a set's slot claim only after the set passes the 8-hour age gate, so run 36177155316 (2026-09-25 19:02 UTC) skipped a claimed `ci-set1` as "only 0 hours old" and never reached the check. `destroy-ci.yml` takes `sweep-min-age-hours` on `main` since PR #363 (`7fa80bde`). During any branch deploy whose claimed set is not last-known-good and has stacks standing, the operator runs `gh workflow run destroy-ci.yml --ref main -f sweep-for-stacks=true -f sweep-min-age-hours=0` (the session's classifier refuses this dispatch), and its log shows "Skipping <set>: its slot claim names a run that is still active". In flight: deploy run 36210037937 on `claude/ops-b30at1-proof` claimed `ci-set1` at 2026-09-26 01:56:22 UTC with `ci-set2` last-known-good; the operator's chained sweep fires at about 02:05:40. **Owner**: Claude Code with the operator. **Model**: Haiku. **Size**: 0 files.
