...
**Prod runs deployment prod-2c66ef3** (PR #353's merge deploy, run 36105457833, 2026-09-25; the confirmation statement activity is listed on ci only).
...
- [ ] **B30bn. The sign-in event Lambda logs errors in prod (issue #355).** `check-prod-env-sign-in-activity-publish-log-errors` went to ALARM at 2026-09-25 07:50:04 UTC on prod `prod-d9ef3c9`, PR #352's deploy with PR #353's environment stacks: `prod-env-sign-in-activity-publish` (`app/functions/auth/signInActivityPublish.js`, invoked fire-and-forget by the Pre Token Generation trigger, so sign-ins are unaffected) logged something its metric filter counts. The SSM names it reads match what `IdentityStack.java` writes (lines 288, 318, 347) and what `ActivityStack.java` grants (lines 228 to 232). Read `/aws/lambda/prod-env-sign-in-activity-publish` for the window (the Logs Insights link in issue #355), name the error, fix the layer (a grant, a missing environment variable such as the security state table, the salt, a bad input shape from the trigger), then close #355 with the evidence. Needs an AWS session (`aws sso login --sso-session diyaccounting`). A pipeline failure, so tier 1. **Owner**: Claude Code. **Model**: Sonnet. **Size**: ~2 files. **In flight**: cause found (`ActivityStack.java` never set `SECURITY_STATE_DYNAMODB_TABLE_NAME` on the Lambda; 103 `ValidationException` errors from 07:49 UTC, so no sign-in event reached the lake); the fix and an `ActivityStackTest` case are `6078b041` on `claude/cyclone-activities`, pushing to PR #354 when its branch deploy finishes; close #355 after the prod deploy shows no more errors.
