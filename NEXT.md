...
- [ ] **B30at1. The sweep's claim check, proven.** The sweep reads a set's slot claim only after the set passes the 8-hour age gate, so run 36177155316 (2026-09-25 19:02 UTC) skipped a claimed `ci-set1` as "only 0 hours old" and never reached the check. PR #363 adds a `sweep-min-age-hours` input to `destroy-ci.yml`. Once it is on `main`, during any branch deploy whose claimed set is not last-known-good and has stacks standing, the operator runs `gh workflow run destroy-ci.yml --ref main -f sweep-for-stacks=true -f sweep-min-age-hours=0` (the session's classifier refuses this dispatch), and its log shows "Skipping <set>: its slot claim names a run that is still active". Blocked on PR #363's merge; machine-ask when it runs. **Owner**: Claude Code with the operator. **Model**: Haiku. **Size**: 0 files.
