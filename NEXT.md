...
- [ ] **B40d.2. Rename the modes to `synthetic`/`live` and give the monitoring vocabulary a
  ...
  **Tracks**: `PLAN_MODE_RENAME.md` is the design; four Sonnet layers, one PR each. Layer 2 (Stripe flag `stripeTestMode` plus migration 004): code complete on `claude/mode-rename-2-stripe-flag`, PR open; verified after the operator runs migration 004 dry then real on ci and prod. Layer 1 (monitoring vocabulary to `probe`, plus the four alarm docs the cuts left stale): worktree `.claude/worktrees/agent-aa7e4815251b66982`, running, stacked on the batch branch. Layers 3 (HMRC mode value, migrations 005 and 006) and 4 (docs and copy) follow in order.
