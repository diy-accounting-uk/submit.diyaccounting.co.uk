...
- [ ] **O1d / G2b. Create the ci GA4 property.** The `ga4-property-sync` dry run for ci is
  clean (property `DIY Accounting Submit (ci)`, stream for
  `https://ci-submit.diyaccounting.co.uk`, BigQuery link to `diyaccounting-ga4` in
  europe-west2, then `SUBMIT_GA4_MEASUREMENT_ID` on the `ci` GitHub Environment). The real run
  from the main session got past the permission classifier on 2026-09-06 and stopped on an
  expired SSO token. **Source**: none. **Owner**: Claude Code. **Model**: Fable (coordinator).
  Blocked on `aws sso login --sso-session diyaccounting`; then the command runs and G2c takes
  the `analytics_<propertyId>` dataset id.
