...
- [ ] **O52i. Keep DIYA's cloud book and set its two prod variables.** B52i's job reads one book, named by `SUBMIT_COMPANY_BOOK_ID` and `SUBMIT_COMPANY_BOOK_OWNER_PREFIX` on GitHub's `prod` environment. DIYA's book is identified (title "DIY Accounting Limited", product `ltd`, 2026-04-01 to 2027-03-31, saved 2026-09-24; the id and owner prefix are with the operator from the session of 2026-09-25, not written here because this repository is public). It is saved with `retention` `sandbox` and expires 2026-10-29, and the job refuses a sandbox book. Steps: the operator re-saves it as a kept book (the rule for kept retention is in `app/functions/diyaGl/diyaGlPut.js`; check it before the re-save); then, with the operator's go, the session runs `gh variable set SUBMIT_COMPANY_BOOK_ID --env prod --body <id>` and the same for the prefix; they take effect at the first `deploy-environment.yml` run after B52i reaches `main`. **Owner**: Claude Code with the operator. **Model**: Haiku. **Size**: 0 files.
