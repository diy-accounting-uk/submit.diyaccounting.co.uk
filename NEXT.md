...
**Prod runs deployment prod-0f4e05c** (PR #354's merge deploy, run 36123087731, 2026-09-25; the confirmation statement activity is listed on ci only).
...
- [ ] **SI-2d. Spreadsheets: DIYA-GL's GA4 sign-in events and tagging.** `PLAN_SIGN_IN_PARITY.md` "SI-2d", in `../spreadsheets.diyaccounting.co.uk`: GA4 `login` and `logout` and `visitor_kind` on the DIYA-GL pages, the sign-out calling Submit's new route. The Submit side is on `main` (PR #353, 2026-09-25): the sign-out route `POST /api/v1/session/sign-out`, the book events and the sign-in events; `main`'s deploy of #353 is green (prod `prod-2c66ef3`). **Owner**: Claude Code. **Model**: Sonnet. **Size**: ~8 files (spreadsheets). **In flight**: `claude/blizzard-signin` in the spreadsheets repository, PR #143 there.

- [ ] **B30bo. The book Lambdas cannot read the app-client-id parameters (issue #357).** `prod-env-cis-unauthorized-api-calls` fired at 2026-09-25 11:47 UTC on 21 denied calls: the `diyaGlPut`, `diyaGlDelete` and `diyaGlVersionGet` roles in `DiyaGlStack.java` were refused `ssm:GetParameter`, because PR #353's book events resolve the app client through `app/lib/appClientResolver.js`, which reads `/submit/<env>/submit-app-client-id`, `spreadsheets-diya-gl-app-client-id` and `mcp-app-client-id`, and only the sign-out and activity-started routes (`AccountStack.java` lines 1043 and 1431) and the sign-in publisher (`ActivityStack.java` line 222) were granted that read. Every book save, open and delete in prod trips the CIS alarm, and its event carries the raw client id instead of the app name. Grant the same `ReadAppClientIdParameters` statement to every Lambda whose handler imports the resolver (`git grep -l appClientResolver -- app/functions`: also `practiceClientBookMovePost`, in `DiyaGlStack.java`), with a CDK test per function, then close #357 with the evidence once prod shows no more denials. The one `macie2:ListCustomDataIdentifiers` denial in the same window came from AWS's own Resource Explorer service role. A pipeline failure, so tier 1. **Owner**: Claude Code. **Model**: Haiku. **Size**: ~2 files. **In flight**: `claude/falcon-grants`, PR #359.

- [ ] **B39a-gate. The Lighthouse gate blocks, at floors the CI runner meets.** The first multi-URL run on `main` (compliance run 36130237719, 2026-09-25) audited all 16 sitemap pages and uploaded the per-page reports, but 10 pages scored below their performance gate on the runner (home 55 against 67, videos 57 against 69, practice 68 against 77, bundles 57 against 62, privacy 72 against 79, and others) while the job passed, because `compliance.yml`'s Lighthouse step carries `continue-on-error: true` (line 251), so no gate is enforced. The floors in `lighthouse.config.json` were measured on a developer machine, which scores higher than the runner. Dispatch `compliance.yml` on `main` two more times (`gh workflow run compliance.yml --ref main`), take each page's lowest performance score across the three runner runs, set its floor 5 points below that (rounded down), leave accessibility, SEO and best practices at the thresholds they meet, remove `continue-on-error`, and prove with a fourth dispatch that the job goes green. The floors are a ratchet: note in the config's comment that they only rise. **Source**: issue #13. **Owner**: Claude Code. **Model**: Haiku. **Size**: ~2 files. **In flight**: merged in PR #358 (`6ef50b96`); the proving compliance run on `main` is 36133620714.
