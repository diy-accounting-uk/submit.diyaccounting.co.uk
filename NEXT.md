...
- [ ] **O52i. Name the company book and set its two prod variables.** B52i's job reads one book, named by `SUBMIT_COMPANY_BOOK_ID` and `SUBMIT_COMPANY_BOOK_OWNER_PREFIX` on GitHub's `prod` environment. The prod bucket `prod-env-diya-gl-972912397388` holds 11 books under 4 owner prefixes, and no record in the workspace names DIYA's (OF2 saved it 2026-09-24). The operator names the book id from the DIYA-GL books page, or signs in so the session reads `GET /api/v1/books`; the owner prefix is then the `users/<prefix>/books/<bookId>/` path (`aws --profile submit-prod s3 ls s3://prod-env-diya-gl-972912397388/users/ --recursive`). With the operator's go, the session runs `gh variable set SUBMIT_COMPANY_BOOK_ID --env prod --body <id>` and the same for the prefix. **Owner**: Claude Code with the operator. **Model**: Haiku. **Size**: 0 files.
