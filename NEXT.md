...
**Prod runs deployment prod-8c41629** (PR #365's merge deploy, run 36217785873 attempt 2, 2026-09-26; the confirmation statement activity is listed on ci only).
...
- [ ] **B30t. The 404 scan detector fails a whole cycle on one S3 `SlowDown` (alarm #366).** `prod-env-scan-detect-404` (`app/functions/security/scanRate404Detect.js`, five-minute schedule in `ScanDetectionStack.java`) raised `SlowDown: Please reduce your request rate` (HTTP 503, 3 attempts) from its `ListObjectsV2` call at line ~71 on 2026-09-26 03:19 UTC, which put `prod-env-scan-detect-404-errors` (Lambda `Errors` >= 1 over 300 s) into ALARM at 03:20; it was OK again by 04:25. Give the module's `S3Client` (line ~34) adaptive retries with more attempts (`retryMode: "adaptive"`, `maxAttempts` around 6), keep a still-failing call an error so a lasting fault still alarms, add a unit test that a first `SlowDown` then success completes the cycle, and close #366 with the cause. **Source**: alarm #366. **Owner**: Claude Code. **Model**: Haiku. **Size**: ~2 files.

## Machine-ask

- [ ] **B30q. Text the operator when prod loses service.** `submit-prod` can send SMS since 2026-09-26: sender ID `DIYACCT` (GB, transactional), the operator's number verified in the End User Messaging sandbox and stored in the SecureString `/submit/prod/operator-sms-number`, and the skill `text-antony-submit-prod` sends one by hand through `pinpoint-sms-voice-v2 send-text-message` (SNS `publish --phone-number` accepted two texts that never arrived; an SNS SMS subscription may fail the same way, so the alarm path needs its delivery proven on a phone, or a small Lambda on the alarm topic that calls `send-text-message`). Remaining: `prod-8c41629-app-alarm-sms-forward` is Active on prod since 2026-09-26 07:5x UTC, subscribed to `prod-8c41629-app-ops-alerts`; the operator runs `aws --profile submit-prod --region eu-west-2 cloudwatch set-alarm-state --alarm-name prod-8c41629-app-health-failed --state-value ALARM --state-reason "SMS path test"` (an AWS write) and confirms the text; the canary returns the alarm to OK at its next run with a recovery text. **Source**: operator, 2026-09-26. **Owner**: Claude Code with the operator. **Model**: Haiku. **Size**: 0 files.
