...
- [ ] **SR-9. `test.yml` does less on documentation-only pushes to `main`.** 52 `test` runs on `main` this session (696 job-minutes, about 13 each) came mostly from `NEXT.md` and plan write-backs. A `paths-ignore` on the `push` trigger will not do: that trigger covers every branch (`test.yml` line 56), and PRs take their required checks (`npm test`, `maven test`, `eslint`) from branch push runs (the comment above line 56), which the `changes` job already reports as skipped on a Markdown-only push. First list which jobs still run on a docs-only `main` push and their minutes (run 36134423480's siblings on `main` give examples), then gate those jobs with `if:` on the `changes` output (and `github.ref == 'refs/heads/main'` where a job must keep running on branches), keeping every required context reported. `prettier --check`, a js-yaml parse and actionlint with CI's flags; prove with one docs-only push after merge. **Source**: session report jCZRKP. **Owner**: Claude Code. **Model**: Sonnet. **Size**: 1 file.
