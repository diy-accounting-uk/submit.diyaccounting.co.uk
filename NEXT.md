...
- [ ] **OSI-1. Four decisions for sign-in parity.** `PLAN_SIGN_IN_PARITY.md` "Open questions": (1) the app client as a new `appClient` field and `app_client` column (recommended) or `site` set to `diya-gl` or `mcp`; (2) active users split by client (recommended) or one total; (3) an in-session `token-refresh` in the lake only (recommended) or also in the LIVE Telegram channel; (4) the MCP's HMRC and Companies House calls on the copied Submit token, leaving `Gov-Client-Multi-Factor` missing, or on the MCP's own sign-in, which also sets its `Gov-Client-Connection-Method`. The operator answers; the session writes the answers into the plan and the SI-2 rows. **Owner**: Operator. **Model**: none. **Size**: 0 files.
...
- [ ] **SI-2a. Submit: sign-in and refresh events from the Pre Token Generation trigger.** `PLAN_SIGN_IN_PARITY.md` "SI-2a": the trigger async-invokes a new `signInActivityPublish` Lambda in `ActivityStack` for every app client; it classifies `login`, `session-resumed` (a refresh more than 65 minutes after the last token issue for that user and client) and `token-refresh`, with `appClient`, `sessionKind` and `sessionId`, state in the security state table; `cognitoTokenPost.js` stops publishing. Merge `claude/arclight-itsa` first (its `itsa-sandbox-year.js` signs in directly on the Submit client). Blocked on OSI-1's answers 1 and 3. **Source**: operator 2026-09-25. **Owner**: Claude Code. **Model**: Sonnet. **Size**: ~18 files.
...
- [ ] **SI-2c. Submit: sign-out, token revocation, book events and the MCP's sign-in tools.** `PLAN_SIGN_IN_PARITY.md` "SI-2c": an authenticated `POST /api/v1/session/sign-out` for all three clients, refresh-token revocation, the beacon's `logout` branch removed, GA4 `logout` and the login `method`; `book-saved`, `book-opened`, `book-deleted` and `book-moved` events; the MCP's `sign_in` and `sign_out` tools. Blocked on SI-2a (the session item). **Owner**: Claude Code. **Model**: Sonnet. **Size**: ~22 files.
