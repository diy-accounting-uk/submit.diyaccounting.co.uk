...
- [ ] **CS-H1. Companies House credit account: prove the new presenter on the live gateway.** Companies House opened the credit account on 2026-09-25 (£500 limit) under a new presenter ID instead of E0000052288; the identifiers are in `../NEXT_OPERATOR_RUNBOOK.md` task A, which the repository does not carry. The operator's choice: file confirmation statements (and accounts) as the new presenter, so the fee lands on that account. A `GetSubmissionStatus` for 06846849 as the new presenter on 2026-09-25 16:40 UTC answered 502 "Authorisation Failure" on both the test service (expected: test filings use test presenter 66666727000) and the live service. The code letter says the code is valid from the next day. On or after 2026-09-26, run `node scripts/companies-house-xmlgw-poll.js --out-dir ../staging/xmlgw-new-presenter` again with `COMPANIES_HOUSE_XMLGW_POLL_COMPANY_NUMBER=06846849`, the new presenter's id and code in `COMPANIES_HOUSE_PRESENTER_ID` and `COMPANIES_HOUSE_PRESENTER_CODE`, and `COMPANIES_HOUSE_GATEWAY_TEST` unset. A status or an empty status list: the new presenter is the live presenter, and O34c and CS-11 name it. 502 again: add the question to CS-H2's email (is the new presenter enabled for software filing, and can it replace E0000052288), or send `../DRAFT_EMAIL_CH_CREDIT_ACCOUNT_LINK.md`. **Source**: `PLAN_COMPANIES_HOUSE_CONFIRMATION_STATEMENT.md` (Tasks table). **Owner**: Claude Code. **Model**: Haiku. **Size**: 0 files.
