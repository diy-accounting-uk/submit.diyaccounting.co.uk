...
|---|---|---|---|---|---|
| 32 | B32.4 (schedule check), B32.5 (ci-only gate) | Optional VAT endpoints: liabilities, payments, penalties are merged and verified on the real sandbox (2026-09-04); add the suites to the 4-hourly schedule | Issue #19, vat-api-operations | S | Revenue, minor. Completes the listed feature set. |
| 30 | B30d (deployed; next alarm), B30h–B30k, O10 (operator) | Cut the alarms and canary runs the audit shows are dead weight | Cost analysis; PLAN_ALARM_CONSOLIDATION | M | Hygiene. Largest recurring CloudWatch line after the composite consolidation. |
...
| 17b | Demo videos for the three VAT read pages: liabilities, payments and penalties, one video each in the 17a capture pattern (`videos/*.json`, `auth: "user"`), recorded against a ci deployment because those activities stay ci-only until the operator has examined them (NEXT.md B32.5), then publish alongside the others. **Ready when** B17a.3 lands, so the pattern is settled on all four existing journeys. | Issue #19 (closed 2026-09-05); 17a | S | Revenue. The pages exist and are on every bundle; a walkthrough is what makes them visible. |
...
| 30a | **Due a week after B30j and B30k land.** Re-run the alarm audit's counts (`_developers/ALARM_AUDIT_2026-09.md`: alarm histories from the CloudWatch API, deploy runs from the GitHub Actions API) over the seven days after both fixes are in prod, and compare families that fired against the 90-day baseline; tune or cut only what that shows. The board's Part 3 gives the same per-family view meanwhile. | Operator decision 2026-09-05; ALARM_AUDIT_2026-09 | S | Trust. Says whether the cuts and the two fixes finished the job, from data rather than a hunch. |
