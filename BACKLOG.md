...
|---|---|---|---|---|---|
| 32 | B32.4 (schedule check) | Optional VAT endpoints: liabilities, payments, penalties are merged and verified on the real sandbox (2026-09-04); add the suites to the 4-hourly schedule | Issue #19, vat-api-operations | S | Revenue, minor. Completes the listed feature set. |
| 30 | B30i, B30j, B30k, B30n, B30o, B30p | Cut the alarms and canary runs the audit shows are dead weight | Cost analysis; `_developers/archive/PLAN_ALARM_CONSOLIDATION.md` | M | Hygiene. Largest recurring CloudWatch line after the composite consolidation. |
...
|---|---|---|---|---|
| 10 | ITSA build, phase 1: sandbox integration with self-employment quarterly update APIs (Business Details, Obligations, SE Business). **In flight**: Business Details is on main; Obligations and the quarterly-update filing are NEXT.md B10.2 and B10.3 on batch 7; the rest splits into one item per endpoint against the paths `_developers/hmrc/ITSA_SPIKE.md` lists (the design doc's paths are for an older API version). Open: confirm the mandate date and income threshold (~£20k proposed), OpenAPI-generated vs hand-rolled client, and token cost per submission (same 1 token as VAT?). | Issues #16, #20; strategy | L | Revenue. The strategic bet. Voluntary sign-up is open now and HMRC auto-enrolment starts September 2026. |
...
| 17b | Demo videos for the three VAT read pages: liabilities, payments and penalties, one video each in the 17a capture pattern (`videos/*.json`, `auth: "user"`), recorded against a ci deployment because those activities stay ci-only until the operator has examined them (NEXT.md B32.5), then publish alongside the others. **Ready now**: B17a.3 landed and the pattern is settled on all four existing journeys. | Issue #19 (closed 2026-09-05); 17a | S | Revenue. The pages exist and are on every bundle; a walkthrough is what makes them visible. |
...
| 40b | Work the PLAN_REDUCE simplification items (in flight on batch 7, NEXT.md B40b) | PLAN_REDUCE | S/M | Hygiene. Standing simplification list; behavior-neutral. Good sub-agent filler. |
