...
| 25 | Cross-account backups, operator steps only: enable cross-account backup at the Organization level, first deploy of the backup-account stacks with the `submit-backup` profile, then the first manual `restore-test.yml` dispatch. The CDK, selection, role, and monthly restore-test workflow are on main | Issue #11, PLAN_CROSS_ACCOUNT_BACKUPS | S | Existential, second layer. The destination exists and denies deletion from outside; the restore test is the gate for the TypeScript migration (#33). |
...
| 43 | Cost optimisation: a whole-bill review across all six AWS accounts — Cost Explorer per account and service, right-sizing, storage classes and lifecycle rules, orphaned resources (us-east-1 log groups and images have recurred), data-transfer lines, and a monthly check that the bill moved the way the month's changes predicted — plus the seventh billing surface added 2026-08-31: the paid Google Cloud account holding the GA4 export, with the stray auto-created `My First Project` (`valued-context-507200-m9`) on the same payment method to confirm empty and delete. Feeds and is fed by #30 (alarms are the known largest CloudWatch line). | Operator, this session; Cowork 2026-08-31 | M | Hygiene, compounding. Nobody has reviewed the whole bill against what runs; every account added since the org split (backup, submit-ci, submit-prod) widened the blind spot. |
...
| 32 | Optional VAT endpoints: liabilities, payments, penalties (#19) | Issue #19, vat-api-operations | M | Revenue, minor. Completes the listed feature set; useful, not urgent. The read-endpoint entitlement gating is 32b. |
