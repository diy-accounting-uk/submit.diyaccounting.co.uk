...
| 17b | Demo videos for the three VAT read pages: liabilities, payments and penalties, one video each in the 17a capture pattern (`videos/*.json`, `auth: "user"`, prod recording), then publish alongside the others. **Ready when** B17a.3 lands, so the pattern is settled on all four existing journeys. | Issue #19 (closed 2026-09-05); 17a | S | Revenue. The pages exist and are on every bundle; a walkthrough is what makes them visible. |
