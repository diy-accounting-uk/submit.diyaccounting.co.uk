...
| 45 | Scheduled security review on Bedrock: rework `security-review.yml` to run the review with an Anthropic model on Bedrock (OIDC role, `bedrock:InvokeModel`), following the pattern of spreadsheets' `app/bin/judge-reconciliation.js` reconciliation judge, replacing the current Copilot-agent flow, then re-enable the weekly cron (line 26, currently commented out) | Operator decision, 2026-08-31 | M | Hygiene. The workflow never runs on its own today, and the Bedrock route needs no third-party agent or long-lived key — same credential stance as the rest of the estate. |
