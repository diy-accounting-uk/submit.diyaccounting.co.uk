...
| 6 ✅ | The route names | `SubmitSharedNames.java` — eight fields and a `PublishedLambda` per route (landed in step 1's commit alongside the bucket, since both are the same one-file edit) | `./mvnw clean verify` regenerates `web/public/docs/api/openapi.json` with the four paths |
