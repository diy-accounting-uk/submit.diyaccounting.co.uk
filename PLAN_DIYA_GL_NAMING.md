...
**The prod bucket holds customer books.** Checked 2026-09-10 19:45 UTC:
`list-object-versions` on `prod-env-books-972912397388` returns current versions under one user
hash, `metadata.json` and `v1.zip` pairs written through that day, and 22 delete markers.
`ci-env-books-367191799875` holds behaviour-run objects and delete markers. The empty result this
plan recorded on 2026-09-09 was true when taken and is no longer.
...
### The bucket: copied, not renamed

S3e's step 1 fired on 2026-09-10. The prod bucket holds customer books, so the plain rename is off
and the copy sequence at the end of this section is the row.

`bucketName` is a replacement property on `AWS::S3::Bucket`, so a changed name makes `DataStack`
create the new bucket and delete the old one, and the old one's objects go with it because
`DataStack.java:655` sets `removalPolicy(DESTROY)` and `autoDeleteObjects(true)`. With books in the
bucket that is data loss, which is why step 1 exists.

The data is backed up: `BackupStack.java:311` puts the books bucket ARN in the critical-resources
selection. A backup is a recovery, not a migration, so the copy sequence still runs.
...
1. Re-run `list-object-versions` against the prod bucket. If it returns anything at all, stop the
   row and report, and run the copy sequence below instead. **This fired on 2026-09-10**, so steps
   2 to 6 do not apply as written and the copy sequence is what runs.
